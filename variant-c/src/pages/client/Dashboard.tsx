...
export default function ClientDashboard() {
 ...
 switch (status) {
 case 'received':
 return <span className="px-2 py-0.5 text-xs font-medium bg-sky-100 text-sky-700">{t('cases.statuses.received')}</span>;
 case 'in_progress':
 return <span className="px-2 py-0.5 text-xs font-medium bg-gray-50 text-amber-700 border border-amber-100">{t('cases.statuses.in_progress')}</span>;
 case 'qc':
 return <span className="px-2 py-0.5 text-xs font-medium bg-violet-100 text-violet-700">{t('cases.statuses.qc')}</span>;
 case 'shipped':
 return <span className="px-2 py-0.5 text-xs font-medium bg-teal-100 text-teal-700">{t('cases.statuses.shipped')}</span>;
 case 'delivered':
 return <span className="px-2 py-0.5 text-xs font-medium bg-gray-50 text-emerald-700 border border-emerald-100">{t('cases.statuses.delivered')}</span>;
 default:
 return <span className="px-2 py-0.5 text-xs font-medium bg-gray-50 text-amber-700 border border-amber-100">{status}</span>;
 }
 };
 ...
 <p className="text-gray-500">{t('common.loadingDashboard')}</p>
 </div>
 </div>
 );
 }
 ...
 <div className=" to-sky-600 px-2 py-1.5 shadow-sky-500/20 flex items-center gap-1.5">
 ...
 <div className=" px-2 py-1.5 shadow-teal-500/20 flex items-center gap-1.5">
 ...
 <div className="bg-gray-800 px-2 py-1.5 shadow-slate-800/20 flex items-center gap-1.5">
 ...
 <h1 className="text-2xl md:text-3xl font-bold text-gray-800 tracking-tight">{t('portal.welcome')}</h1>
 <p className="text-xs md:text-sm text-gray-500">{user?.client?.studioName || t('portal.defaultClient')}</p>
 </div>
 ...
 <Link to="/portal/cases" className="text-sm text-blue-600 font-semibold hover:text-sky-700 transition-colors">
 ...
 <span className="font-semibold text-gray-800 truncate">{delivery.patient}</span>
 ...
 <span className="text-gray-400"> · {delivery.teethCount} {t('dental.tooth', { count: delivery.teethCount })}</span>
 )}
 </p>
 <p className="text-[10px] font-mono text-gray-400 mt-0.5">#{delivery.caseNumber}</p>
 </div>
 <div className="flex items-center gap-3 ml-3">
 <div className="text-right">
 <p className="text-[10px] text-gray-400">{t('portal.deliveryLabel', { defaultValue: 'Consegna' })}</p>
 ...
 <p className="text-gray-400">{t('portal.noDeliveriesScheduled', { defaultValue: 'Nessuna consegna in programma' })}</p>
 </div>
 )}
 </div>
 </div>
 ...
 !msg.isFromMe ? 'bg-gray-50 border-l-4 border-gray-500 hover:bg-sky-100' : 'bg-gray-50 hover:bg-gray-100'
 }`}
 >
 <div className="flex items-center justify-between mb-1">
 <span className="text-sm font-semibold text-gray-800 truncate">{msg.patient}</span>
 <span className="text-xs text-gray-400 shrink-0 ml-2">{formatRelativeTime(msg.createdAt)}</span>
 </div>
 ...
 <p className="text-[10px] font-mono text-gray-400 mt-1">#{msg.caseNumber}</p>
 </Link>
 )) : (
 <p className="text-sm text-gray-400 text-center py-4">{t('portal.noMessages', { defaultValue: 'Nessun messaggio' })}</p>
 )}
 </div>
 </div>
 </div>
