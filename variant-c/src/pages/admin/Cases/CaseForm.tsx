...
const WORK_TYPES = [
 ...
 { id: 'implant', name: 'dental.workTypes.impianto', color: 'bg-blue-500', price: 250 },
 { id: 'bridge', name: 'dental.workTypes.ponte', color: 'bg-gray-600', price: 350 },
 { id: 'veneer', name: 'dental.workTypes.faccetta', color: 'bg-pink-500', price: 220 },
 { id: 'inlay', name: 'dental.workTypes.intarsio', color: 'bg-orange-500', price: 150 },
...
export default function CaseForm() {
 ...
 <div className="animate-spin h-10 w-10 border-2 border-blue-600 border-t-transparent mx-auto mb-4"></div>
 ...
 className="inline-flex items-center gap-2 px-4 py-2.5 to-teal-500 text-white text-sm font-medium hover: hover:opacity-90 transition font-display"
 >
 <Sparkles size={16} />
 ...
 {/* Main Card */}
 <div className="bg-white border border-gray-100 p-5 sm:p-6 space-y-6">
 ...
 <h2 className="text-base font-semibold text-gray-800 flex items-center gap-2 pb-3 border-b border-gray-100 font-display">
 ...
 <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">{t('cases.client')} *</label>
 <button
 type="button"
 ...
 {showClientDropdown && (
 <div className="absolute z-20 w-full mt-1 bg-white -lg border border-gray-100 py-2 max-h-60 overflow-y-auto">
 {loadingClients ? (
 <div className="px-3 py-2 text-gray-500 text-center text-sm">{t('common.loading')}</div>
 ) : clients.length === 0 ? (
 <div className="px-3 py-2 text-gray-500 text-center text-sm">{t('cases.noClientsFound')}</div>
 ) : (
 clients.map(client => (
 <button
 key={client.id}
 ...
 >
 <p className="text-sm font-medium text-gray-800">{client.studioName}</p>
 ...
 {/* Dentista */}
 <div className="relative">
 <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">{t('cases.operatingDentist')}</label>
 <button
 type="button"
 ...
 >
 <span className={selectedDentist ? 'text-gray-800' : 'text-gray-400'}>
 ...
 className="w-full px-3 py-2 text-left hover:bg-gray-50 text-gray-400 text-sm"
 >
 {t('cases.noDentist')}
 </button>
 ...
 {dentist.specialization && <p className="text-xs text-gray-500">{dentist.specialization}</p>}
 </button>
 ))}
 </div>
 )}
 ...
 {/* Paziente */}
 <div>
 <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">{t('newCase.patientName')}</label>
 <input
 type="text"
 value={patientName}
 ...
 {/* Avvisi paziente */}
 <div>
 <label className="block text-xs font-semibold text-gray-600 mb-1.5 flex items-center gap-1.5 uppercase tracking-wider">
 ...
 <div className="space-y-4">
 <div>
 <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">{t('cases.receivedDate')}</label>
 <input
 type="date"
 value={receivedDate}
 ...
 <div>
 <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">{t('cases.dueDate')}</label>
 <input
 type="date"
 value={dueDate}
 ...
 <label className="text-[10px] font-semibold text-gray-500 shrink-0 uppercase tracking-wider">{t('cases.priority')}</label>
 <div className="flex gap-1 flex-1">
 {[
 { value: 'normal', label: t('cases.priorities.normal'), activeCls: 'bg-gray-50 text-emerald-700 ring-1 ring-emerald-500' },
 ...
 className={`flex-1 px-2 py-1.5 text-[10px] font-semibold transition-all ${
 priority === option.value ? option.activeCls : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
 }`}
 >
 {option.label}
 </button>
 ...
 {/* Note lavorazione */}
 <div>
 <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">Note lavorazione</label>
 <textarea
 value={notes}
 ...
 <span className="ml-auto text-xs bg-blue-500 text-white px-2 py-0.5 font-semibold">
 ...
 className="border-2 border-dashed border-gray-300 p-4 text-center hover:border-blue-500 hover:bg-blue-500/5 transition-colors cursor-pointer"
 >
 <div className="flex items-center justify-center gap-2">
 <Upload size={18} className="text-gray-400" />
 <div className="text-left">
 <p className="text-gray-700 text-xs font-medium">{t('files.dragDrop')}</p>
 <p className="text-[10px] text-gray-400">JPG, PNG, STL, PLY</p>
 </div>
 </div>
 </div>

 {/* Lista file */}
 <div className="bg-gray-50 p-2 min-h-[100px] max-h-[200px] overflow-y-auto border border-gray-100">
 {(selectedFiles.length + uploadedFiles.length) === 0 ? (
 ...
 return (
 <div key={`new-${index}`} className="flex items-center justify-between p-2 bg-white border border-gray-100 ">
 <div className="flex items-center gap-2 min-w-0">
 <div className="w-7 h-7 bg-gray-50 flex items-center justify-center flex-shrink-0">
 <FileIcon size={12} />
 </div>
 <div className="min-w-0">
 <p className="text-[11px] font-medium text-gray-700 truncate max-w-[110px]">{file.name}</p>
 ...
 return (
 <div key={file.id} className="flex items-center justify-between p-2 bg-white border border-gray-100 ">
 <div className="flex items-center gap-2 min-w-0">
 <div className={`w-7 h-7 flex items-center justify-center flex-shrink-0 ${
 is3D ? 'bg-gray-50 text-blue-600' : isImage ? 'bg-gray-50 text-blue-600' : 'bg-gray-100 text-gray-600'
 }`}>
 <FileIcon size={12} />
 </div>
 <div className="min-w-0">
 <p className="text-[11px] font-medium text-gray-700 truncate max-w-[100px]">{file.name}</p>
 <p className="text-[9px] text-gray-400">{file.size}</p>
 </div>
 </div>
 <div className="flex items-center gap-0.5 flex-shrink-0">
 {isImage && (
 <button type="button" onClick={() => openImagePreview(file)} className="text-gray-400 hover:text-blue-600 p-0.5 transition-colors" title={t('cases.imagePreview')}>
 <Eye size={12} />
 </button>
 )}
 {is3D && (
 <button type="button" onClick={() => open3DPreview(file)} className="text-gray-400 hover:text-blue-600 p-0.5 transition-colors" title={t('viewer3d.viewIn3D')}>
 <Scan size={12} />
 </button>
 )}
 <button type="button" onClick={() => removeFile(file.id)} className="text-gray-400 hover:text-red-500 p-0.5 transition-colors" title={t('viewer3d.deleteFile')}>
 <Trash2 size={12} />
 </button>
 </div>
 </div>
 ...
 <h2 className="text-base font-semibold text-gray-800 flex items-center gap-2 font-display">
 {t('dental.selectTooth')} *
 <span className="text-[10px] font-normal text-gray-400 hidden sm:inline">
 · <kbd className="px-1.5 py-0.5 bg-gray-100 border border-gray-200 text-[9px] font-mono">CTRL</kbd> + click = range
 </span>
 </h2>
 ...
 {t(type.name)}
 <span className={`text-[9px] ${currentWorkType.id === type.id ? 'text-white/70' : 'text-gray-400'}`}>₪{type.price}</span>
 </button>
 ))}
 </div>
 </div>

 <div className="bg-gray-50 p-3 sm:p-6 border border-gray-100">
 ...
 <span className="text-[10px] sm:text-xs text-gray-400 font-semibold uppercase tracking-wider">FDI</span>
 <div className="flex-1 h-px bg-gray-300" />
 </div>
 ...
 className="flex items-center justify-between gap-2 bg-white border border-gray-100 px-3 py-2 "
 >
 <div className="flex items-center gap-2 min-w-0">
 <div className={`w-2.5 h-2.5 shrink-0 ${group.workType.color}`} />
 ...
 <div className="flex items-center gap-2">
 <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">{t('cases.total', { defaultValue: 'Totale' })}</span>
 <span className="text-xl font-bold text-gray-800 font-display">₪{calculateTotal()}</span>
 {selectedTeeth.length > 0 && (
 <span className="text-[10px] text-gray-400">({selectedTeeth.length} {t('dental.tooth', { count: selectedTeeth.length })})</span>
 )}
 </div>
 <div className="flex items-center gap-2">
 <Link
 to="/admin/cases"
 className="inline-flex items-center gap-1 px-4 py-2.5 text-gray-600 font-medium text-xs hover:bg-gray-100 transition-colors"
 >
 <ArrowLeft size={12} />
 Annulla
 </Link>
 ...
 className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-blue-600 text-white font-medium text-xs hover:bg-sky-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed "
 >
 <Save size={14} />
 ...
 <div className="bg-white p-5 md:p-6 border-2 border-red-100 ">
 <h3 className="font-semibold text-gray-800 mb-2 font-display flex items-center gap-2">
 ...
 disabled={loading}
 className="px-4 py-2.5 bg-red-600 text-white font-medium hover:bg-red-700 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm "
 >
 <Trash2 size={16} />
 Elimina Caso
 </button>
