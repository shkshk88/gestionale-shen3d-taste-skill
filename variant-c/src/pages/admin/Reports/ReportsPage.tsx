...
export default function ReportsPage() {
 ...
 { name: 'Clinica Dentale Rossi', revenue: 12450, cases: 45, color: 'bg-blue-500' },
 { name: 'Studio Dr. Verdi', revenue: 9820, cases: 32, color: 'bg-blue-500' },
 { name: 'Dental Care Center', revenue: 8340, cases: 28, color: 'bg-gray-700' },
 { name: 'Smile Center Ferrari', revenue: 6200, cases: 22, color: 'bg-gray-500' },
 { name: 'Studio Bianchi', revenue: 4890, cases: 18, color: 'bg-blue-600' },
 ];
 ...
 { id: 'emax', name: t('dental.materialsShort.EMAX'), count: 52, percentage: 27, color: 'bg-blue-500' },
 { id: 'mc', name: t('dental.materialsShort.METAL_CERAMIC'), count: 38, percentage: 20, color: 'bg-gray-700' },
 { id: 'pmma', name: t('dental.materials.PMMA'), count: 22, percentage: 11, color: 'bg-gray-500' },
 { id: 'comp', name: t('dental.materials.COMP'), count: 14, percentage: 7, color: 'bg-blue-600' },
 ];

 const casesByStatus = [
 { key: 'completed', status: t('reports.status_completed'), count: 124, color: 'bg-blue-500', percentage: 79 },
 { key: 'inProgress', status: t('reports.status_inProgress'), count: 22, color: 'bg-blue-500', percentage: 14 },
 { key: 'pending', status: t('reports.status_pending'), count: 8, color: 'bg-orange-500', percentage: 5 },
 ...
 <p className="text-sm text-gray-500">{t('reports.insightDesc')}</p>
 </div>
 ...
 <button className="px-4 py-2 border border-gray-200 text-gray-600 hover:bg-gray-50 transition-all flex items-center gap-2">
 <Download size={18} />
 {t('common.export')}
 </button>
 </div>
 ...
 stats.revenue.trend === 'up' ? 'text-blue-600' : 'text-red-600'
 }`}>
 {stats.revenue.trend === 'up' ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
 {stats.revenue.change}%
 </span>
 </div>
 <p className="text-sm text-gray-500 mb-1">{t('reports.revenue')}</p>
 <p className="text-2xl font-bold text-gray-800">₪{stats.revenue.value.toLocaleString('he-IL')}</p>
 </div>
 ...
 <span className={`flex items-center gap-1 text-sm font-medium ${
 stats.cases.trend === 'up' ? 'text-blue-600' : 'text-red-600'
 }`}>
 {stats.cases.trend === 'up' ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
 {stats.cases.change}%
 </span>
 </div>
 <p className="text-sm text-gray-500 mb-1">{t('reports.completedCases')}</p>
 <p className="text-2xl font-bold text-gray-800">{stats.cases.value}</p>
 </div>

 {/* Average Value */}
 <div className="card-base p-5">
 <div className="flex items-start justify-between mb-3">
 <div className="w-11 h-11 bg-gray-800/20 flex items-center justify-center">
 <BarChart3 size={22} className="text-gray-800" />
 </div>
 <span className={`flex items-center gap-1 text-sm font-medium ${
 stats.avgValue.trend === 'up' ? 'text-blue-600' : 'text-red-600'
 }`}>
 {stats.avgValue.trend === 'up' ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
 {Math.abs(stats.avgValue.change)}%
 </span>
 </div>
 <p className="text-sm text-gray-500 mb-1">{t('reports.avgCaseValue')}</p>
 <p className="text-2xl font-bold text-gray-800">₪{stats.avgValue.value}</p>
 </div>

 {/* Active Clients */}
 <div className="card-base p-5">
 <div className="flex items-start justify-between mb-3">
 <div className="w-11 h-11 bg-gray-500/20 flex items-center justify-center">
 <Users size={22} className="text-gray-500" />
 </div>
 <span className="text-sm font-medium text-gray-400">—</span>
 </div>
 <p className="text-sm text-gray-500 mb-1">{t('clients.activeClients')}</p>
 <p className="text-2xl font-bold text-gray-800">{stats.clients.value}</p>
 </div>
 </div>
 ...
 <div>
 <h3 className="font-semibold text-gray-800">Andamento Fatturato</h3>
 <p className="text-sm text-gray-500">Ultimi 6 mesi</p>
 </div>
 ...
 <span className="text-gray-600">Casi</span>
 </div>
 </div>
 </div>
 ...
 {/* Revenue Bar */}
 <div
 className="w-full max-w-[40px] bg-blue-600 rounded-t-lg transition-all hover:opacity-80 relative group"
 ...
 <span className="text-sm text-gray-500">{t(`calendar.shortMonths.${data.monthKey}`)}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="flex items-center justify-between mb-6">
 <h3 className="font-semibold text-gray-800">Casi per Stato</h3>
 ...
 <div className="card-base p-6">
 <div className="flex items-center justify-between mb-6">
 <h3 className="font-semibold text-gray-800">Top Clienti per Fatturato</h3>
 <button className="text-sm text-blue-600 hover:underline">Vedi tutti</button>
 </div>

 <div className="space-y-4">
 {topClients.map((client, index) => (
 <div key={client.name} className="flex items-center gap-4">
 <div className={`w-10 h-10 ${rankColors[index] || 'bg-gray-400'} flex items-center justify-center text-white font-bold text-sm`}>
 {index + 1}
 </div>
 <div className="flex-1 min-w-0">
 <p className="font-medium text-gray-800 truncate">{client.name}</p>
 <p className="text-sm text-gray-500">{t('reports.casesCount', { count: client.cases })}</p>
 </div>
 <div className="text-right">
 <p className="font-semibold text-gray-800">₪{client.revenue.toLocaleString('he-IL')}</p>
 ...
 <div className="card-base p-6">
 <div className="flex items-center justify-between mb-6">
 <h3 className="font-semibold text-gray-800">Utilizzo Materiali</h3>
 ...
 <div className="flex items-center gap-2">
 <div className={`w-3 h-3 ${material.color}`} />
 <span className="text-sm font-medium text-gray-700">{material.name}</span>
 </div>
 <span className="text-sm text-gray-500">{material.count} {t('reports.units')} ({material.percentage}%)</span>
 </div>
 <div className="h-2 bg-gray-50 overflow-hidden">
 <div
 className={`h-full ${material.color} transition-all`}
 ...
 <span className="text-sm font-medium text-gray-600">{t('reports.totalWork')}</span>
 ...
 {/* Quick Insights */}
 <div className=" /80 p-6 text-white ">
