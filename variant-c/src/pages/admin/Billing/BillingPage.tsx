...
function StatusBadge({ status }: { status: DocumentStatus }) {
 ...
 issued: { label: t('billing.status.issued'), cn: 'bg-gray-50 text-sky-700' },
 paid: { label: t('billing.status.paid'), cn: 'bg-gray-50 text-teal-700' },
 cancelled: { label: t('billing.status.cancelled'), cn: 'bg-gray-100 text-gray-500' },
 error: { label: t('billing.status.error'), cn: 'bg-red-100 text-red-700' },
 };
 const { label, cn } = cfg[status] ?? { label: status, cn: 'bg-gray-100 text-gray-500' };
 return (
 <span className={`inline-flex px-2 py-0.5 text-[10px] font-semibold ${cn}`}>
 {label}
 </span>
 );
}
...
export default function BillingPage() {
 ...
 {/* Banner Fase B */}
 <div className="card-base from-sky-50 via-sky-50/50 to-transparent border-sky-200 p-3 flex items-start gap-3">
 <div className="w-8 h-8 bg-sky-100 flex items-center justify-center shrink-0">
 ...
 className={`p-2.5 border transition-all text-left ${
 isActive
 ...
 <div
 className={`w-8 h-8 ${tab.accentBg} flex items-center justify-center shrink-0`}
 >
 ...
 {tab.hebrew && (
 <p className="text-[10px] text-gray-400 truncate leading-tight" dir="rtl">
 {tab.hebrew}
 </p>
 )}
 </div>
 ...
 {activeTab !== 'unbilled' && (
 <div className="flex items-center gap-1 bg-gray-50 px-2 h-9">
 ...
 className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium flex items-center gap-1.5 shrink-0 transition-colors"
 >
 ...
 className="px-3 py-1.5 bg-blue-600 text-white text-sm font-medium hover:bg-sky-700 flex items-center gap-1.5 shrink-0 transition-colors"
 >
 <Plus size={14} />
 ...
 className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-medium transition-colors"
 >
 <Download size={12} />
 ...
 className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 hover:bg-teal-100 text-teal-700 text-xs font-medium transition-colors"
 >
 ...
 className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 hover:bg-sky-100 text-sky-700 text-xs font-medium transition-colors"
 >
 ...
 <Loader2 size={28} className="mx-auto animate-spin text-blue-600 mb-3" />
 <p className="text-sm text-gray-500">{t('billing.loading_unbilled')}</p>
 </div>
 ) : filteredGroups.length === 0 ? (
 <div className="text-center py-16 border-2 border-dashed border-gray-200 ">
 <div className="w-14 h-14 bg-gray-50 mx-auto mb-3 flex items-center justify-center">
 <Inbox size={24} className="text-amber-700" />
 </div>
 <p className="text-sm font-semibold text-gray-700">{t('billing.empty_unbilled_title')}</p>
 ...
 className="flex items-center gap-3 p-3 cursor-pointer hover:bg-gray-50/60 transition"
 >
 <button
 className="w-7 h-7 bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200 shrink-0 transition-colors"
 ...
 className="px-3 py-2 bg-gray-50 hover:bg-teal-100 text-teal-700 text-xs font-semibold inline-flex items-center gap-1.5 transition-colors"
 ...
 className="px-3 py-2 bg-blue-600 hover:bg-sky-700 text-white text-xs font-semibold inline-flex items-center gap-1.5 transition-colors"
 ...
 <tr
 key={c.id}
 className="border-t border-gray-100 hover:bg-white/60"
 >
 ...
 <div className="flex items-center justify-end gap-3 pt-2 px-1 text-xs text-gray-500 border-t border-gray-100">
 <span className="pt-2">
 <strong className="text-gray-800">{filteredGroups.length}</strong>{' '}
 {t(`billing.summary.clients_${filteredGroups.length === 1 ? 'one' : 'other'}`)}
 </span>
 <span className="text-gray-300 pt-2">·</span>
 <span className="pt-2">
 <strong className="text-gray-800">{totalCases}</strong>{' '}
 ...
 <div className="text-center py-16">
 <Loader2 size={28} className="mx-auto animate-spin text-blue-600 mb-3" />
 <p className="text-sm text-gray-500">{t('billing.loading_documents')}</p>
 </div>
 ) : filteredDocs.length === 0 ? (
 <div className="text-center py-16 border-2 border-dashed border-gray-200 ">
 <div
 className={`w-14 h-14 ${active.accentBg} mx-auto mb-3 flex items-center justify-center`}
 >
 ...
 <p className="text-lg font-bold text-gray-800">{filteredDocs.length}</p>
 </div>
 <div className="bg-gray-50 p-3 text-center">
 <p className="text-[10px] uppercase text-blue-600 font-semibold tracking-wider">
 {t('billing.summary.total')}
 </p>
 ...
 <div
 key={doc.id}
 className="flex items-center gap-3 p-3 border border-gray-100 hover:bg-gray-50/60 transition "
 >
 {/* Left: status + info */}
 ...
 {doc.documentNumber ?? (
 <span className="font-normal text-gray-400 font-sans">{t('billing.status.draft')}</span>
 )}
 </span>
 {doc.invoice4uEnvironment && (
 <span
 className={`text-[9px] px-1.5 py-0.5 font-bold uppercase tracking-wide ${
 ...
 {doc.invoice4uError && (
 <span
 className="text-[9px] px-1.5 py-0.5 font-bold uppercase bg-red-100 text-red-700"
 ...
 size={18}
 />
 <span className="text-xs text-gray-700" dir="auto">{doc.client.studioName}</span>
 {doc.subject && (
 ...
 <p className="text-[10px] text-gray-400">IVA {doc.taxRate}%</p>
 </div>
 ...
 className="px-2.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-semibold inline-flex items-center gap-1 shrink-0 transition-colors"
 title={
 ...
 <button className="w-8 h-8 hover:bg-gray-100 flex items-center justify-center text-gray-500 shrink-0 transition-colors">
 ...
 <h3 className="font-semibold text-gray-800 mb-2">{confirm.title}</h3>
 <p className="text-sm text-gray-600 mb-5">{confirm.message}</p>
 ...
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 ">
 <div className="bg-white p-5 max-w-sm w-full space-y-3">
 <div className="flex items-center justify-between">
 <h3 className="font-semibold text-gray-800">{t('billing.actions.edit_draft')}</h3>
 <button
 onClick={() => setEditModal((p) => ({ ...p, open: false }))}
 className="w-7 h-7 hover:bg-gray-100 flex items-center justify-center text-gray-400 transition-colors"
 >
 ×
 </button>
 </div>
 <div>
 <label className="text-xs font-medium text-gray-600 mb-1 block">{t('billing.create_modal.subject_label')}</label>
 <input
 type="text"
 ...
 <div>
 <label className="text-xs font-medium text-gray-600 mb-1 block">{t('common.notes', { defaultValue: 'Note' })}</label>
 <textarea
 ...
 <div>
 <label className="text-xs font-medium text-gray-600 mb-1 block">{t('cases.dueDate')}</label>
 <input
 type="date"
 ...
 className="flex-1 py-2.5 bg-blue-600 text-white text-sm font-semibold hover:bg-sky-700 disabled:opacity-50 flex items-center justify-center gap-2 transition-colors"
 >
