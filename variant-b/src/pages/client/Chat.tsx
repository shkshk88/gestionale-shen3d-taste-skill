...
export default function ClientChat() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <Loader2 className="w-8 h-8 animate-spin text-amber-800 mx-auto mb-4" />
  ...
  if (cases.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <p className="text-stone-500 font-medium">{t('chat.noActiveCases')}</p>
        <p className="text-sm text-stone-400">Non ci sono casi aperti per chattare</p>
        <Link
          to="/portal/new-case"
          className="px-4 py-2 bg-stone-800 text-white rounded-xl font-semibold text-sm shadow-sm hover:bg-stone-700 transition-all"
        >
  ...
  return (
    <div className="h-[calc(100vh-120px)] md:h-[calc(100vh-140px)] flex flex-col md:flex-row gap-4 -mx-2 px-2 md:mx-0 md:px-0">
      ...
      >
        <h2 className="text-sm font-bold text-stone-700 uppercase tracking-wider mb-1 px-1">
          {t('chat.openCases')}
        </h2>
        {cases.map((c) => (
          <button
            ...
            className={`text-left w-full p-3 rounded-xl transition-all duration-200 flex items-center gap-3 shadow-sm border ${
              selectedCaseId === c.id
                ? 'bg-gradient-to-br from-slate-700 to-slate-800 text-white border-slate-700'
            ...
            <div className="min-w-0">
              <p
                className={`text-sm font-semibold truncate ${
                  selectedCaseId === c.id ? 'text-white' : 'text-stone-800'
                }`}
              >
                {c.caseNumber}
              </p>
              <p
                className={`text-xs truncate ${
                  selectedCaseId === c.id ? 'text-slate-300' : 'text-stone-500'
                }`}
              >
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="md:hidden flex items-center gap-2 mb-2">
            <button
              ...
              className="w-8 h-8 rounded-lg bg-white border border-stone-200 flex items-center justify-center text-stone-600 shadow-sm"
            >
              <ChevronLeft size={18} />
            </button>
