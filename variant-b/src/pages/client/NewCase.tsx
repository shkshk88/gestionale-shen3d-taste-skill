...
const WORK_TYPES = [
  { id: 'zirconia', name: 'dental.workTypes.coronaZirconia', color: 'bg-amber-700', material: 'ZR', workType: 'corona' },
  ...
  { id: 'implant', name: 'dental.workTypes.impianto', color: 'bg-green-700', material: 'ZR', workType: 'impianto' },
  ...
  { id: 'temporary', name: 'dental.workTypes.provvisorio', color: 'bg-stone-400', material: 'PMMA', workType: 'altro' },
];
...
export default function NewCase() {
  ...
  const getToothColor = (toothNumber: number): string => {
    ...
    return tooth ? tooth.workType.color : 'bg-stone-200 hover:bg-slate-300';
  };
  ...
  const getFileIcon = (filename: string) => {
    ...
    return <FileBox size={20} className="text-amber-800" />;
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-stone-500">{t('newCase.loadingUserData')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6 animate-fade-in max-w-4xl mx-auto p-4">
        <div className="flex items-center gap-4">
          <Link
            to="/portal"
            className="w-10 h-10 rounded-xl bg-white shadow-sm border border-stone-100 flex items-center justify-center text-stone-500 hover:text-stone-800 transition-all"
          >
            <ArrowLeft size={20} />
          </Link>
          <h1 className="text-2xl font-bold text-stone-800">{t('newCase.title')}</h1>
        </div>
        ...
        <div className="bg-white rounded-xl border border-stone-100 shadow-sm p-8 text-center">
          ...
          <div className="bg-stone-100 rounded-xl p-4 mb-6 text-left font-mono text-xs overflow-auto max-h-40">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              ...
              className="px-6 py-3 bg-amber-800 text-white rounded-xl font-medium hover:bg-sky-700 transition-all"
            >
  ...
  return (
    <div className="space-y-3 animate-fade-in max-w-6xl mx-auto p-2 sm:p-4">
      <div className="bg-white rounded-xl border border-stone-100 shadow-sm p-4 sm:p-6 space-y-4">

        {/* Top section: 3 columns */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Column 1: Patient Data */}
          <div className="space-y-4">
            <h2 className="text-base font-semibold text-stone-800 flex items-center gap-2 pb-2 border-b border-stone-100">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  <Stethoscope size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400" />
                  {dentists.length === 0 && (
                    <div className="input-modern w-full text-sm pl-9 bg-stone-100 text-stone-500">
                      {t('newCase.noDentists')}
                    </div>
                  )}
                  {dentists.length === 1 && (
                    <div className="input-modern w-full text-sm pl-9 bg-stone-50 text-stone-700">
                      {dentists[0].name}
                      {dentists[0].specialization && (
                        <span className="text-stone-500 ml-1">({dentists[0].specialization})</span>
                      )}
                    </div>
                  )}
                  {dentists.length > 1 && (
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  disabled
                  className="input-modern w-full text-sm bg-stone-100 text-stone-500 cursor-not-allowed"
                />
                <p className="text-[10px] text-stone-400 mt-1">Data automatica di oggi</p>
              </div>
                ...
                <div className="flex gap-1 flex-1">
                  <button
                    ...
                    className={`flex-1 px-2 py-1 rounded-md text-[10px] font-medium transition-all flex items-center justify-center gap-1 ${
                      priority === 'normal'
                        ? 'bg-green-50 text-teal-700 ring-1 ring-teal-500'
                  ...
                  <button
                    ...
                    className={`flex-1 px-2 py-1 rounded-md text-[10px] font-medium transition-all flex items-center justify-center gap-1 ${
                      priority === 'urgent'
                        ? 'bg-orange-50 text-amber-700 ring-1 ring-amber-500'
                        : 'bg-stone-100 text-stone-600 hover:bg-stone-200'
                    }`}
                  >
          ...
          <div className="space-y-4">
            <h2 className="text-base font-semibold text-stone-800 flex items-center gap-2 pb-2 border-b border-stone-100">
              <Box size={18} className="text-green-700" />
              File Allegati
              {files.length > 0 && (
                <span className="ml-auto text-xs bg-amber-800 text-white px-2 py-0.5 rounded-full">
                  {files.length}
                </span>
              )}
            </h2>
            ...
            <div>
              ...
              <div
                ...
                className="border-2 border-dashed border-stone-300 rounded-xl p-3 text-center hover:border-amber-500 hover:bg-orange-50/50 transition-colors cursor-pointer"
              >
                <div className="flex items-center justify-center gap-2">
                  <Upload size={16} className="text-stone-400" />
                  <div className="text-left">
                    <p className="text-stone-700 text-xs font-medium">Clicca o trascina file</p>
                    <p className="text-[10px] text-stone-400">JPG, PNG, STL, PLY - Max 20MB</p>
                  </div>
                </div>
              </div>
            </div>

            <div className="bg-stone-100 rounded-xl p-3 min-h-[100px] max-h-[160px] overflow-y-auto">
              {files.length === 0 ? (
                ...
                <div className="space-y-2">
                  {files.map((fileObj) => (
                    <div key={fileObj.id} className="flex items-center justify-between p-2 bg-white rounded-lg shadow-sm border border-stone-100">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-7 h-7 rounded-md bg-stone-100 flex items-center justify-center flex-shrink-0">
                          {getFileIcon(fileObj.file.name)}
                        </div>
                        <div className="min-w-0">
                          <p className="text-xs font-medium text-stone-700 truncate max-w-[120px]">{fileObj.file.name}</p>
                          <p className="text-[10px] text-stone-400">{formatFileSize(fileObj.file.size)}</p>
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={() => handleRemoveFile(fileObj.id)}
                        className="text-stone-400 hover:text-red-500 p-1 flex-shrink-0"
                      >
                        <X size={12} />
                      </button>
                    </div>
            ...
            {uploadProgress > 0 && uploadProgress < 100 && (
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-stone-600">Caricamento...</span>
                  <span className="text-stone-800 font-medium">{uploadProgress}%</span>
                </div>
                <div className="h-1.5 bg-stone-200 rounded-full overflow-hidden">
                  <div className="h-full bg-amber-800 transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-stone-100 rounded-xl p-2 sm:p-6">
            ...
            <div className="flex items-center gap-2 sm:gap-4 my-2">
              ...
              <span className="text-[10px] sm:text-xs text-stone-400 font-medium">FDI</span>
              <div className="flex-1 h-0.5 bg-slate-300" />
            </div>
              ...
              <div className="space-y-1.5">
                {Object.values(
                  ...
                  <div
                    ...
                    className="flex items-center justify-between gap-2 bg-white rounded-lg shadow-sm border border-stone-100 px-2.5 py-1.5"
                  >
                    <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <p className="text-[10px] text-stone-400 flex items-center gap-1">
            <span className="text-stone-500">💡</span> <kbd className="px-1 py-0.5 bg-stone-100 border rounded text-[9px]">CTRL</kbd> + click per selezionare range
          </p>
          <div className="flex items-center gap-2">
            <Link
              to="/portal"
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-stone-600 hover:text-stone-800 hover:bg-stone-100 transition-all"
            >
              <ArrowLeft size={12} />
              Annulla
            </Link>
            <button
              ...
              className="flex items-center gap-1 px-4 py-1.5 bg-amber-800 text-white rounded-lg text-xs font-semibold hover:bg-sky-700 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Check size={12} />
