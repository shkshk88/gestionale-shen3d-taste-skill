...
function StatusBadge({ status }: { status: DocumentStatus }) {
  ...
  const cfg: Record<DocumentStatus, { label: string; cn: string }> = {
    ...
    issued: { label: t('billing.status.issued'), cn: 'bg-orange-50 text-sky-700' },
    paid: { label: t('billing.status.paid'), cn: 'bg-green-50 text-teal-700' },
    cancelled: { label: t('billing.status.cancelled'), cn: 'bg-stone-100 text-stone-500' },
    error: { label: t('billing.status.error'), cn: 'bg-red-100 text-red-700' },
  };
  const { label, cn } = cfg[status] ?? { label: status, cn: 'bg-stone-100 text-stone-500' };
  return (
    ...
    <div className="space-y-4 animate-fade-in pb-8 max-w-7xl mx-auto p-2 sm:p-4">
          ...
          return (
            <button
              ...
              className={`p-2.5 rounded-xl border transition-all text-left ${
                isActive
                  ? 'border-amber-800 ring-2 ring-amber-800/20 shadow-soft bg-white'
                  : 'border-stone-100 hover:border-stone-200 hover:shadow-soft bg-white'
              }`}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {tab.hebrew && (
                    <p className="text-[10px] text-stone-400 truncate leading-tight" dir="rtl">
                      {tab.hebrew}
                    </p>
                  )}
                </div>
      ...
      <div className="card-base p-4 sm:p-5 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          {activeTab !== 'unbilled' && (
            <div className="flex items-center gap-1 bg-stone-50 rounded-xl px-2 h-9">
          ...
          {activeTab === 'unbilled' ? (
            <button
              ...
              className="px-3 py-1.5 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-lg text-sm font-medium flex items-center gap-1.5 shrink-0 transition-colors"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 bg-amber-800 text-white rounded-lg text-sm font-medium hover:bg-sky-700 flex items-center gap-1.5 shrink-0 transition-colors"
            >
              <Plus size={14} />
        ...
        {activeTab !== 'unbilled' && (
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-50 hover:bg-teal-100 text-teal-700 rounded-lg text-xs font-medium transition-colors"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 hover:bg-sky-100 text-sky-700 rounded-lg text-xs font-medium transition-colors"
            >
        ...
        {activeTab === 'unbilled' ? (
          loadingUnbilled ? (
            <div className="text-center py-16">
              <Loader2 size={28} className="mx-auto animate-spin text-amber-800 mb-3" />
              <p className="text-sm text-stone-500">{t('billing.loading_unbilled')}</p>
            </div>
          ) : filteredGroups.length === 0 ? (
            <div className="text-center py-16 border-2 border-dashed border-stone-200 rounded-xl">
              <div className="w-14 h-14 rounded-xl bg-orange-50 mx-auto mb-3 flex items-center justify-center">
                <Inbox size={24} className="text-amber-700" />
              </div>
              <p className="text-sm font-semibold text-stone-700">{t('billing.empty_unbilled_title')}</p>
            ...
            <>
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        className="flex items-center gap-3 p-3 cursor-pointer hover:bg-stone-50/60 transition"
                      >
                        <button
                          className="w-7 h-7 rounded-lg bg-stone-100 flex items-center justify-center text-stone-500 hover:bg-stone-200 shrink-0 transition-colors"
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className="font-semibold text-sm text-stone-800 truncate"
                            dir="auto"
                        ...
                        <div className="flex gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
                          <button
                            ...
                            className="px-3 py-2 bg-green-50 hover:bg-teal-100 text-teal-700 rounded-xl text-xs font-semibold inline-flex items-center gap-1.5 transition-colors"
                          ...
                          <button
                            ...
                            className="px-3 py-2 bg-amber-800 hover:bg-sky-700 text-white rounded-xl text-xs font-semibold inline-flex items-center gap-1.5 transition-colors"
                          ...
                          <table className="w-full text-xs">
                            ...
                            <tbody>
                              {g.cases.map((c) => (
                                <tr
                                  key={c.id}
                                  className="border-t border-stone-100 hover:bg-white/60"
                                >
              ...
              <div className="flex items-center justify-end gap-3 pt-2 px-1 text-xs text-stone-500 border-t border-stone-100">
                <span className="pt-2">
                  <strong className="text-stone-800">{filteredGroups.length}</strong>{' '}
                  {t(`billing.summary.clients_${filteredGroups.length === 1 ? 'one' : 'other'}`)}
                </span>
                <span className="text-slate-300 pt-2">·</span>
                <span className="pt-2">
                  <strong className="text-stone-800">{totalCases}</strong>{' '}
          ...
          <>
            {loadingDocs ? (
              <div className="text-center py-16">
                <Loader2 size={28} className="mx-auto animate-spin text-amber-800 mb-3" />
                <p className="text-sm text-stone-500">{t('billing.loading_documents')}</p>
              </div>
            ) : filteredDocs.length === 0 ? (
              <div className="text-center py-16 border-2 border-dashed border-stone-200 rounded-xl">
                <div
                  className={`w-14 h-14 rounded-xl ${active.accentBg} mx-auto mb-3 flex items-center justify-center`}
                >
                ...
                <div className="flex items-baseline justify-center gap-2 mb-1">
                  ...
                  {active.hebrew && (
                    <p className="text-xs text-stone-500" dir="rtl">
                      ({active.hebrew})
                    </p>
                  )}
                </div>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-stone-50 rounded-xl p-3 text-center">
                    <p className="text-[10px] uppercase text-stone-500 font-medium tracking-wider">
                      {t('billing.tabs.invoice_order')}
                    </p>
                    <p className="text-lg font-bold text-stone-800">{filteredDocs.length}</p>
                  </div>
                  <div className="bg-orange-50 rounded-xl p-3 text-center">
                    <p className="text-[10px] uppercase text-amber-800 font-semibold tracking-wider">
                      {t('billing.summary.total')}
                    </p>
                ...
                <div className="space-y-2">
                  {filteredDocs.map((doc) => (
                    <div
                      key={doc.id}
                      className="flex items-center gap-3 p-3 border border-stone-100 rounded-xl hover:bg-stone-50/60 transition shadow-soft"
                    >
                      {/* Left: status + info */}
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="font-mono text-sm font-semibold text-stone-800">
                            {doc.documentNumber ?? (
                              <span className="font-normal text-stone-400 font-sans">{t('billing.status.draft')}</span>
                            )}
                          </span>
                        ...
                        <div className="flex items-center gap-1.5">
                          <ClientAvatar
                            ...
                            size={18}
                          />
                          <span className="text-xs text-stone-700" dir="auto">{doc.client.studioName}</span>
                          {doc.subject && (
                      ...
                      <div className="text-right shrink-0 mr-1">
                        ...
                        <p className="text-[10px] text-stone-400">IVA {doc.taxRate}%</p>
                      </div>
      ...
      {confirm.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-elevated p-5 max-w-sm w-full">
            <h3 className="font-semibold text-stone-800 mb-2">{confirm.title}</h3>
            <p className="text-sm text-stone-600 mb-5">{confirm.message}</p>
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setConfirm((p) => ({ ...p, open: false }))}
                className="px-4 py-2 rounded-xl border border-stone-200 text-sm text-stone-600 hover:bg-stone-50 transition-colors"
              >
                {t('common.cancel')}
              </button>
              <button
      ...
      {editModal.open && editModal.doc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-elevated p-5 max-w-sm w-full space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-stone-800">{t('billing.actions.edit_draft')}</h3>
              <button
                onClick={() => setEditModal((p) => ({ ...p, open: false }))}
                className="w-7 h-7 rounded-full hover:bg-stone-100 flex items-center justify-center text-stone-400 transition-colors"
              >
                ×
              </button>
            </div>
            <div>
              <label className="text-xs font-medium text-stone-600 mb-1 block">{t('billing.create_modal.subject_label')}</label>
              <input
                type="text"
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-amber-800 text-white text-sm font-semibold hover:bg-sky-700 disabled:opacity-50 flex items-center justify-center gap-2 transition-colors"
              >
