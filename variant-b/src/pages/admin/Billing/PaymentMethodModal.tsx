...
const METHOD_DEFS: {
  ...
  { type: 4, i18nKey: 'cash', icon: Banknote, color: 'text-emerald-700', bg: 'bg-green-50 border-emerald-200' },
  { type: 3, i18nKey: 'bank_transfer', icon: Building, color: 'text-sky-700', bg: 'bg-orange-50 border-sky-200' },
  { type: 2, i18nKey: 'check', icon: FileCheck, color: 'text-amber-700', bg: 'bg-orange-50 border-amber-200' },
...
export default function PaymentMethodModal({ open, onClose, doc, onConfirm }: Props) {
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-xl shadow-xl overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex items-center gap-3 p-4 border-b border-stone-100 shrink-0">
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-stone-500 uppercase tracking-wide font-semibold">
        ...
        <div className="p-4 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-2">
            {METHOD_DEFS.map((m) => {
              ...
              return (
                ...
                >
                  <Icon size={18} className={isActive ? m.color : 'text-stone-400'} />
                  <p
                    className={`text-sm font-semibold mt-1 ${
                      isActive ? m.color : 'text-stone-700'
                    }`}
                  >
          ...
          {/* Method-specific fields */}
          {method === 2 && (
            <div className="space-y-2 p-3 bg-orange-50/40 rounded-xl border border-amber-100">
              <input
                type="text"
          ...
          {method === 3 && (
            <div className="space-y-2 p-3 bg-orange-50/40 rounded-xl border border-sky-100">
              <input
                type="text"
                placeholder={t('billing.payment_modal.bank')}
                value={bankName}
          ...
          {method === 1 && (
            <div className="space-y-2 p-3 bg-violet-50/40 rounded-xl border border-violet-100">
              <input
                type="text"
          ...
          {method === 4 && (
            <p className="text-xs text-emerald-700 px-3 py-2 bg-green-50/40 rounded-xl border border-emerald-100">
        ...
        {/* Footer */}
        <div className="border-t border-stone-100 p-4 bg-stone-50/60 shrink-0 flex gap-2">
          <button
            onClick={onClose}
          ...
          <button
            ...
            disabled={loading}
            className="flex-[2] py-2.5 rounded-xl bg-amber-800 hover:opacity-90 text-white text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-50"
          >
