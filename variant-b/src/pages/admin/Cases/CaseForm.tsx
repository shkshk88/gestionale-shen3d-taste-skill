...
const WORK_TYPES = [
  { id: 'zirconia', name: 'dental.workTypes.coronaZirconia', color: 'bg-amber-700', price: 180 },
  ...
  { id: 'implant', name: 'dental.workTypes.impianto', color: 'bg-green-700', price: 250 },
  { id: 'bridge', name: 'dental.workTypes.ponte', color: 'bg-stone-600', price: 350 },
  { id: 'veneer', name: 'dental.workTypes.faccetta', color: 'bg-pink-500', price: 220 },
  { id: 'inlay', name: 'dental.workTypes.intarsio', color: 'bg-orange-600', price: 150 },
  { id: 'temporary', name: 'dental.workTypes.provvisorio', color: 'bg-stone-400', price: 50 },
];
...
export default function CaseForm() {
  ...
  const getToothColor = (toothNumber: number): string => {
    ...
    return 'bg-stone-200 hover:bg-slate-300';
  };
  ...
  const ToothButton = ({ number }: { number: number }) => (
    <button
      ...
      className={`aspect-[3/4] w-full min-w-0 rounded-lg sm:rounded-xl ${getToothColor(number)} text-[10px] sm:text-xs font-medium transition-all duration-200 flex items-center justify-center ${
        selectedTeeth.find(t => t.number === number) ? 'text-white shadow-md scale-105' : 'text-stone-600'
      }`}
    >
      {number}
    </button>
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          <div className="animate-spin rounded-full h-10 w-10 border-2 border-amber-800 border-t-transparent mx-auto mb-4"></div>
  ...
  return (
    <div className="space-y-5 animate-fade-in pb-8 max-w-6xl mx-auto">
      ...
      {/* Main Card */}
      <div className="bg-white rounded-xl border border-stone-100 shadow-soft p-5 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            <h2 className="text-base font-semibold text-stone-800 flex items-center gap-2 pb-3 border-b border-stone-100 font-display">
              <span className="w-8 h-8 rounded-lg bg-green-50 text-green-800 flex items-center justify-center">
                <Building2 size={16} />
              </span>
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <label className="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wider">{t('cases.client')} *</label>
                <button
                  type="button"
                  onClick={() => setShowClientDropdown(!showClientDropdown)}
                  className="input-modern w-full text-left flex items-center justify-between text-sm"
                >
                  <span className={selectedClient ? 'text-stone-800' : 'text-stone-400'}>
                    {selectedClient
                ...
                {showClientDropdown && (
                  <div className="absolute z-20 w-full mt-1 bg-white rounded-xl shadow-soft-lg border border-stone-100 py-2 max-h-60 overflow-y-auto">
                    {loadingClients ? (
                      <div className="px-3 py-2 text-stone-500 text-center text-sm">{t('common.loading')}</div>
                    ) : clients.length === 0 ? (
                      <div className="px-3 py-2 text-stone-500 text-center text-sm">{t('cases.noClientsFound')}</div>
                    ) : (
                      clients.map(client => (
                        <button
                          key={client.id}
                        ...
                        >
                          <p className="text-sm font-medium text-stone-800">{client.studioName}</p>
                          <p className="text-xs text-stone-500">{client.contactPerson}</p>
                        </button>
                      ))
                    )}
                  </div>
              ...
              {/* Dentista */}
              <div className="relative">
                <label className="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wider">{t('cases.operatingDentist')}</label>
                <button
                  type="button"
                ...
                >
                  <span className={selectedDentist ? 'text-stone-800' : 'text-stone-400'}>
                ...
                {showDentistDropdown && dentists.length > 0 && (
                  <div className="absolute z-20 w-full mt-1 bg-white rounded-xl shadow-soft-lg border border-stone-100 py-2 max-h-60 overflow-y-auto">
                    <button
                      ...
                      className="w-full px-3 py-2 text-left hover:bg-stone-50 text-stone-400 text-sm"
                    >
                      {t('cases.noDentist')}
                    </button>
                    {dentists.map(dentist => (
                      <button
                        ...
                        className="w-full px-3 py-2.5 text-left hover:bg-stone-50 transition-colors"
                      >
                        <p className="text-sm font-medium text-stone-800">{dentist.name}</p>
                        {dentist.specialization && <p className="text-xs text-stone-500">{dentist.specialization}</p>}
                      </button>
                    ))}
                  </div>
                )}
              ...
              {/* Paziente */}
              <div>
                <label className="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wider">{t('newCase.patientName')}</label>
                <input
                  type="text"
                  value={patientName}
              ...
              {/* Avvisi paziente */}
              <div>
                <label className="block text-xs font-semibold text-stone-600 mb-1.5 flex items-center gap-1.5 uppercase tracking-wider">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wider">{t('cases.receivedDate')}</label>
                <input
                  type="date"
                  value={receivedDate}
              ...
              <div>
                <label className="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wider">{t('cases.dueDate')}</label>
                <input
                  type="date"
                  value={dueDate}
              ...
              <div className="flex items-center gap-2">
                <label className="text-[10px] font-semibold text-stone-500 shrink-0 uppercase tracking-wider">{t('cases.priority')}</label>
                <div className="flex gap-1 flex-1">
                  {[
                    { value: 'normal', label: t('cases.priorities.normal'), activeCls: 'bg-green-50 text-emerald-700 ring-1 ring-emerald-500' },
                    ...
                    <button
                      ...
                      className={`flex-1 px-2 py-1.5 rounded-lg text-[10px] font-semibold transition-all ${
                        priority === option.value ? option.activeCls : 'bg-stone-50 text-stone-600 hover:bg-stone-100'
                      }`}
                    >
                      {option.label}
                    </button>
              ...
              {/* Note lavorazione */}
              <div>
                <label className="block text-xs font-semibold text-stone-600 mb-1.5 uppercase tracking-wider">Note lavorazione</label>
                <textarea
                  value={notes}
          ...
          <div className="space-y-5">
            <h2 className="text-base font-semibold text-stone-800 flex items-center gap-2 pb-3 border-b border-stone-100 font-display">
              ...
              {(selectedFiles.length + uploadedFiles.length) > 0 && (
                <span className="ml-auto text-xs bg-green-700 text-white px-2 py-0.5 rounded-full font-semibold">
            ...
            <div
              ...
              className="border-2 border-dashed border-stone-300 rounded-xl p-4 text-center hover:border-green-700 hover:bg-green-700/5 transition-colors cursor-pointer"
            >
              <div className="flex items-center justify-center gap-2">
                <Upload size={18} className="text-stone-400" />
                <div className="text-left">
                  <p className="text-stone-700 text-xs font-medium">{t('files.dragDrop')}</p>
                  <p className="text-[10px] text-stone-400">JPG, PNG, STL, PLY</p>
                </div>
              </div>
            </div>

            {/* Lista file */}
            <div className="bg-stone-50 rounded-xl p-2 min-h-[100px] max-h-[200px] overflow-y-auto border border-stone-100">
              {(selectedFiles.length + uploadedFiles.length) === 0 ? (
                ...
                <div className="space-y-1.5">
                  ...
                  {selectedFiles.map((file, index) => {
                    ...
                    return (
                      <div key={`new-${index}`} className="flex items-center justify-between p-2 bg-white rounded-xl border border-stone-100 shadow-sm">
                        <div className="flex items-center gap-2 min-w-0">
                          <div className="w-7 h-7 rounded-lg bg-stone-50 flex items-center justify-center flex-shrink-0">
                            <FileIcon size={12} />
                          </div>
                          <div className="min-w-0">
                            <p className="text-[11px] font-medium text-stone-700 truncate max-w-[110px]">{file.name}</p>
                            <p className="text-[9px] text-stone-400">{formatFileSize(file.size)}</p>
                          </div>
                        </div>
                        <button
                          type="button"
                          onClick={() => handleRemoveNewFile(index)}
                          className="text-stone-400 hover:text-red-500 p-0.5 flex-shrink-0 transition-colors"
                        >
                          <X size={12} />
                        </button>
                      </div>
                  ...
                  {uploadedFiles.map(file => {
                    ...
                    return (
                      <div key={file.id} className="flex items-center justify-between p-2 bg-white rounded-xl border border-stone-100 shadow-sm">
                        <div className="flex items-center gap-2 min-w-0">
                          <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${
                            is3D ? 'bg-orange-50 text-amber-800' : isImage ? 'bg-green-50 text-green-800' : 'bg-stone-100 text-stone-600'
                          }`}>
                            <FileIcon size={12} />
                          </div>
                          <div className="min-w-0">
                            <p className="text-[11px] font-medium text-stone-700 truncate max-w-[100px]">{file.name}</p>
                            <p className="text-[9px] text-stone-400">{file.size}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-0.5 flex-shrink-0">
                          {isImage && (
                            <button type="button" onClick={() => openImagePreview(file)} className="text-stone-400 hover:text-green-800 p-0.5 transition-colors" title={t('cases.imagePreview')}>
                              <Eye size={12} />
                            </button>
                          )}
                          {is3D && (
                            <button type="button" onClick={() => open3DPreview(file)} className="text-stone-400 hover:text-amber-800 p-0.5 transition-colors" title={t('viewer3d.viewIn3D')}>
                              <Scan size={12} />
                            </button>
                          )}
                          <button type="button" onClick={() => removeFile(file.id)} className="text-stone-400 hover:text-red-500 p-0.5 transition-colors" title={t('viewer3d.deleteFile')}>
                            <Trash2 size={12} />
                          </button>
                        </div>
                      </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <h2 className="text-base font-semibold text-stone-800 flex items-center gap-2 font-display">
              {t('dental.selectTooth')} *
              <span className="text-[10px] font-normal text-stone-400 hidden sm:inline">
                · <kbd className="px-1.5 py-0.5 bg-stone-100 border border-stone-200 rounded-lg text-[9px] font-mono">CTRL</kbd> + click = range
              </span>
            </h2>
            <div className="flex flex-wrap gap-1.5">
              {WORK_TYPES.map(type => (
                ...
                >
                  ...
                  {t(type.name)}
                  <span className={`text-[9px] ${currentWorkType.id === type.id ? 'text-white/70' : 'text-stone-400'}`}>₪{type.price}</span>
                </button>
              ))}
            </div>
          </div>

          <div className="bg-stone-50 rounded-xl p-3 sm:p-6 border border-stone-100">
            ...
            <div className="flex items-center gap-2 sm:gap-4 my-2">
              ...
              <span className="text-[10px] sm:text-xs text-stone-400 font-semibold uppercase tracking-wider">FDI</span>
              <div className="flex-1 h-px bg-slate-300" />
            </div>
              ...
              <div className="space-y-1.5">
                {Object.values(
                  ...
                  <div
                    ...
                    className="flex items-center justify-between gap-2 bg-white rounded-xl border border-stone-100 px-3 py-2 shadow-sm"
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${group.workType.color}`} />
                      <span className="text-xs font-medium text-stone-700 truncate">{t(group.workType.name)}</span>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-xs font-mono text-stone-500">{summarizeToothRanges(group.numbers)}</span>
                      <button
                        type="button"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider">{t('cases.total', { defaultValue: 'Totale' })}</span>
            <span className="text-xl font-bold text-stone-800 font-display">₪{calculateTotal()}</span>
            {selectedTeeth.length > 0 && (
              <span className="text-[10px] text-stone-400">({selectedTeeth.length} {t('dental.tooth', { count: selectedTeeth.length })})</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            <Link
              to="/admin/cases"
              className="inline-flex items-center gap-1 px-4 py-2.5 text-stone-600 rounded-xl font-medium text-xs hover:bg-stone-100 transition-colors"
            >
              <ArrowLeft size={12} />
              Annulla
            </Link>
            <button
              ...
              className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-amber-800 text-white rounded-xl font-medium text-xs hover:bg-sky-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
            >
              <Save size={14} />
      ...
      {isEditing && (
        <div className="mt-6 pt-6 border-t border-stone-100">
          <div className="bg-white rounded-xl p-5 md:p-6 border-2 border-red-100 shadow-soft">
            <h3 className="font-semibold text-stone-800 mb-2 font-display flex items-center gap-2">
