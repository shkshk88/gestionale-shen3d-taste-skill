        ...
        {!result && (
          <>
            <div className="flex items-center gap-3 pb-3 border-b border-stone-100">
              <div className="w-10 h-10 rounded-xl bg-green-50 flex items-center justify-center ring-1 ring-teal-500/20">
                <Wand2 size={18} className="text-green-800" />
              </div>
            ...
            <div
              ...
              className="border-2 border-dashed border-stone-200 rounded-xl p-8 text-center hover:border-teal-400 hover:bg-green-50/30 transition-all cursor-pointer"
            >
              <div className="w-14 h-14 bg-stone-50 rounded-xl flex items-center justify-center mx-auto mb-4 ring-1 ring-slate-100">
              ...
              <p className="text-[11px] text-stone-400 mt-2 font-medium">JPG, PNG, WEBP, HEIC · max 10MB</p>
            </div>

            {images.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                {images.map((img) => (
                  <div key={img.id} className="relative rounded-xl overflow-hidden bg-stone-100 aspect-square group ring-1 ring-slate-200">
                    <img src={img.previewUrl} alt="" className="w-full h-full object-cover" />
                    <button
                      type="button"
                      onClick={() => removeImage(img.id)}
                      className="absolute top-2 right-2 w-7 h-7 rounded-full bg-stone-800/70 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-stone-800"
                    >
                      <X size={14} />
                    </button>
                  </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-3 border-t border-stone-100">
              <p className="text-[11px] text-stone-400 flex-1 leading-relaxed">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-5 py-2.5 bg-green-700 text-white rounded-xl text-sm font-semibold hover:bg-green-800 transition-all shadow-soft disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {analysing ? (
                  <>
          ...
          <>
            <div className="flex items-center gap-3 pb-3 border-b border-stone-100">
              <button
                ...
                className="w-9 h-9 rounded-xl bg-stone-50 flex items-center justify-center hover:bg-stone-200 transition ring-1 ring-slate-200"
              ...
              <span className={`text-[11px] font-bold px-3 py-1 rounded-full ${
                lowConfidence ? 'bg-orange-50 text-amber-700 ring-1 ring-amber-500/20' : 'bg-green-50 text-teal-700 ring-1 ring-teal-500/20'
              }`}>
                AI {confidencePct}%
              </span>
            </div>
            ...
            {lowConfidence && (
              <div className="flex items-start gap-3 p-4 bg-orange-50 border border-amber-200 rounded-xl ring-1 ring-amber-500/10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {result.studioName && (
                    <p className="text-[10px] text-stone-400 mt-1">
                      AI ha letto: <span className="italic text-stone-600">{result.studioName}</span>
                    </p>
                  )}
                </div>
                ...
                {result.dentistName && (
                  <div className="p-3 bg-stone-50 rounded-xl ring-1 ring-slate-100">
                    <p className="text-[10px] text-stone-500 font-medium uppercase tracking-wide">Dentista letto da AI</p>
                    <p className="text-sm font-semibold text-stone-700 mt-1">{result.dentistName}</p>
                    <p className="text-[10px] text-stone-400 italic mt-0.5">Assegnalo manualmente nel caso se serve</p>
                  </div>
                )}
                ...
                {result.notes && (
                  <div>
                    ...
                    <p className="text-xs text-stone-700 p-3 bg-stone-50 rounded-xl italic leading-relaxed ring-1 ring-slate-100">{result.notes}</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                {result.teeth.length === 0 ? (
                  <p className="text-xs text-stone-400 p-5 text-center bg-stone-50 rounded-xl ring-1 ring-slate-100">
                    Nessun dente rilevato. Aggiungi manualmente dopo la creazione del caso.
                  </p>
                ) : (
                  <div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
                    {result.teeth.map((tooth, idx) => (
                      <div key={`${tooth.toothNumber}-${idx}`} className="flex items-center gap-3 p-3 bg-stone-50 rounded-xl ring-1 ring-slate-100 hover:shadow-soft transition-shadow">
                        <span className="font-mono text-sm font-bold text-stone-800 w-9 h-9 flex items-center justify-center bg-white rounded-xl ring-1 ring-slate-200 shrink-0">
                          {tooth.toothNumber}
                        </span>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs text-stone-700 font-medium">
                            {t(`dental.workTypes.${tooth.workType}`, { defaultValue: tooth.workType })}
                            <span className="text-stone-400 font-normal"> · {t(`dental.materials.${tooth.material}`, { defaultValue: tooth.material })}</span>
                          </p>
                          {tooth.vitaColor && (
                            <p className="text-[10px] text-stone-500 mt-0.5">Colore: {tooth.vitaColor}</p>
                          )}
                          {tooth.notes && (
                            <p className="text-[10px] text-stone-400 italic mt-0.5">{tooth.notes}</p>
                          )}
                        </div>
                      </div>
                    ))}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-4 border-t border-stone-100">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setResult(null)}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-stone-600 hover:bg-stone-50 transition-all ring-1 ring-slate-200"
                >
                  <FileImage size={14} />
                  Ricarica foto
                </button>
                <button
                  ...
                  className="flex items-center gap-1.5 px-5 py-2 bg-green-700 text-white rounded-xl text-xs font-semibold hover:bg-green-800 transition-all shadow-soft disabled:opacity-50 disabled:cursor-not-allowed"
                >
