  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-surface md:bg-grain">
      ...
      <aside className="hidden md:flex fixed start-3 top-3 bottom-3 w-[72px] bg-white/80 backdrop-blur-xl border border-stone-200/50 rounded-[1.5rem] flex-col items-center py-5 gap-1 z-50 shadow-elevated">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Tooltip */}
                <div className="absolute start-full ms-3 px-2.5 py-1.5 bg-stone-800 text-white text-xs font-medium rounded-lg opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all pointer-events-none whitespace-nowrap z-[60]">
                  {t(item.label)}
                </div>
              </NavLink>
            );
        ...
        <button
          onClick={logout}
          className="w-10 h-10 flex items-center justify-center text-stone-400 hover:text-red-500/80 hover:bg-red-50/60 rounded-xl transition-all duration-200"
          title={t('auth.logout')}
        >
      ...
      <main className="flex-1 flex flex-col mx-2 md:ms-[88px] md:me-3 my-2 md:my-3 mb-24 md:mb-3 overflow-hidden relative">
        ...
        {/* Header */}
        <header className="h-[68px] mb-3 bg-white/70 backdrop-blur-xl border border-stone-200/50 rounded-[1.5rem] px-4 md:px-5 flex items-center justify-between z-40 shrink-0 shadow-soft">
          {/* Page Title */}
          <div className="flex flex-col min-w-0">
            <h1 className="text-lg md:text-xl font-semibold text-stone-800 font-display tracking-tight truncate">{pageInfo.title}</h1>
            {pageInfo.subtitle && (
              <p className="text-xs text-stone-500 mt-0.5 font-medium hidden md:block">{pageInfo.subtitle}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="relative group hidden md:block">
              <Search className="absolute start-3 top-1/2 -translate-y-1/2 text-stone-400 group-focus-within:text-brand transition-colors" size={15} />
              <input
                type="text"
                placeholder={t('common.search')}
                className="ps-9 pe-3 py-1.5 w-40 bg-surface-secondary hover:bg-white focus:bg-white rounded-full border border-transparent focus:border-brand/20 text-sm focus:outline-none focus:ring-2 focus:ring-brand/10 placeholder:text-stone-400 transition-all duration-300"
              />
            </div>
            ...
            <div className="hidden md:flex items-center gap-2.5 ps-2 cursor-pointer hover:opacity-80 transition-opacity">
              <div className="text-end hidden md:block">
                <p className="text-sm font-semibold text-stone-700">{user?.name || 'Admin'}</p>
                <p className="text-[10px] text-stone-400 font-semibold uppercase tracking-wider">{user?.role || 'admin'}</p>
              </div>
