  ...
  if (cases.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <p className="text-slate-500 font-medium">{t('chat.noActiveCases')}</p>
        <p className="text-sm text-slate-400">Non ci sono casi aperti per chattare</p>
        <Link
          to="/portal/new-case"
          className="px-4 py-2 bg-slate-800 text-white rounded-xl font-semibold text-sm shadow-sm hover:bg-slate-700 transition-all"
        >
      ...
      >
        ...
        {cases.map((c) => (
          <button
            ...
            className={`text-left w-full p-3 rounded-2xl transition-all duration-200 flex items-center gap-3 shadow-sm border ${
              selectedCaseId === c.id
            ...
            <div className="min-w-0">
              ...
              <p
                className={`text-xs truncate ${
                  selectedCaseId === c.id ? 'text-slate-300' : 'text-slate-500'
                }`}
              >
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="md:hidden flex items-center gap-2 mb-2">
            <button
              ...
              className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-600 shadow-sm"
            >
              <ChevronLeft size={18} />
            </button>
