  ...
  const ToggleSwitch = ({ checked, onChange }: { checked: boolean; onChange: (val: boolean) => void }) => (
    <button
      ...
      className={`relative w-12 h-6 rounded-full transition-all duration-300 ${
        checked ? 'bg-slate-800 shadow-sm shadow-slate-800/20' : 'bg-slate-200'
      }`}
    >
      <div
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-slate-500">{t('common.loadingProfile')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 animate-scale-in max-w-4xl pb-4">
      ...
      <div className="flex items-center gap-3">
        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-sky-500 to-slate-800 flex items-center justify-center text-white font-bold text-lg shadow-sm shadow-sky-500/15 shrink-0">
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4">
        <div className="bg-gradient-to-br from-slate-700 to-slate-800 rounded-2xl md:rounded-[1.5rem] p-3 md:p-5 shadow-sm shadow-slate-800/15">
        ...
        <div className="bg-gradient-to-br from-sky-500 to-sky-600 rounded-2xl md:rounded-[1.5rem] p-3 md:p-5 shadow-sm shadow-sky-500/15">
          <div className="flex items-center gap-2 md:gap-3 mb-1 md:mb-2">
            <div className="w-6 h-6 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-white/20 flex items-center justify-center">
        ...
        <div className="bg-gradient-to-br from-teal-500 to-teal-600 rounded-2xl md:rounded-[1.5rem] p-3 md:p-5 shadow-sm shadow-teal-500/15">
          <div className="flex items-center gap-2 md:gap-3 mb-1 md:mb-2">
            <div className="w-6 h-6 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-white/20 flex items-center justify-center">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${
                i18n.language === lang.code
                  ? 'bg-slate-800 text-white shadow-sm shadow-slate-800/15'
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
        ...
        <div className="space-y-2">
          <button className="w-full flex items-center justify-between p-4 bg-slate-50 rounded-xl hover:bg-slate-100 transition-all duration-200 group">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-white flex items-center justify-center shadow-sm border border-slate-100">
                <Shield size={18} className="text-slate-500 group-hover:text-sky-600 transition-colors" />
              </div>
          ...
          <button className="w-full flex items-center justify-between p-4 bg-slate-50 rounded-xl hover:bg-slate-100 transition-all duration-200 group">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-white flex items-center justify-center shadow-sm border border-slate-100">
                <Clock size={18} className="text-slate-500 group-hover:text-sky-600 transition-colors" />
              </div>
      ...
      {/* Contact Lab */}
      <div className="bg-gradient-to-br from-sky-500 to-slate-800 rounded-2xl p-6 shadow-sm shadow-sky-500/15">
