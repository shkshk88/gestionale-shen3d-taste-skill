...
const WORK_TYPES = [
  { id: 'zirconia', name: 'dental.workTypes.coronaZirconia', color: 'bg-sky-500', material: 'ZR', workType: 'corona' },
  ...
  { id: 'implant', name: 'dental.workTypes.impianto', color: 'bg-teal-500', material: 'ZR', workType: 'impianto' },
  ...
  { id: 'temporary', name: 'dental.workTypes.provvisorio', color: 'bg-slate-400', material: 'PMMA', workType: 'altro' },
];
...
export default function NewCase() {
  ...
  const getToothColor = (toothNumber: number): string => {
    ...
    return tooth ? tooth.workType.color : 'bg-slate-200 hover:bg-slate-300';
  };
  ...
  const getFileIcon = (filename: string) => {
    ...
    return <FileBox size={20} className="text-sky-600" />;
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-slate-500">{t('newCase.loadingUserData')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6 animate-fade-in max-w-4xl mx-auto p-4">
        <div className="flex items-center gap-4">
          <Link
            to="/portal"
            className="w-10 h-10 rounded-xl bg-white shadow-sm border border-slate-100 flex items-center justify-center text-slate-500 hover:text-slate-800 transition-all"
          >
            <ArrowLeft size={20} />
          </Link>
          <h1 className="text-2xl font-bold text-slate-800">{t('newCase.title')}</h1>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              ...
              className="px-6 py-3 bg-sky-600 text-white rounded-xl font-medium hover:bg-sky-700 transition-all"
            >
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  {dentists.length === 0 && (
                    <div className="input-modern w-full text-sm pl-9 bg-slate-100 text-slate-500">
                      {t('newCase.noDentists')}
                    </div>
                  )}
                  {dentists.length === 1 && (
                    <div className="input-modern w-full text-sm pl-9 bg-slate-50 text-slate-700">
                      {dentists[0].name}
                      {dentists[0].specialization && (
                        <span className="text-slate-500 ml-1">({dentists[0].specialization})</span>
                      )}
                    </div>
                  )}
                  {dentists.length > 1 && (
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  disabled
                  className="input-modern w-full text-sm bg-slate-100 text-slate-500 cursor-not-allowed"
                />
                <p className="text-[10px] text-slate-400 mt-1">Data automatica di oggi</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <label className="text-[10px] font-medium text-slate-500 shrink-0">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div
                ...
                className="border-2 border-dashed border-slate-300 rounded-xl p-3 text-center hover:border-sky-500 hover:bg-sky-50/50 transition-colors cursor-pointer"
              >
                <div className="flex items-center justify-center gap-2">
                  <Upload size={16} className="text-slate-400" />
                  <div className="text-left">
                    <p className="text-slate-700 text-xs font-medium">Clicca o trascina file</p>
                    <p className="text-[10px] text-slate-400">JPG, PNG, STL, PLY - Max 20MB</p>
                  </div>
                </div>
              </div>
            </div>

            <div className="bg-slate-100 rounded-xl p-3 min-h-[100px] max-h-[160px] overflow-y-auto">
              {files.length === 0 ? (
                ...
                <div className="space-y-2">
                  {files.map((fileObj) => (
                    <div key={fileObj.id} className="flex items-center justify-between p-2 bg-white rounded-lg shadow-sm border border-slate-100">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-7 h-7 rounded-md bg-slate-100 flex items-center justify-center flex-shrink-0">
                          {getFileIcon(fileObj.file.name)}
                        </div>
                        <div className="min-w-0">
                          <p className="text-xs font-medium text-slate-700 truncate max-w-[120px]">{fileObj.file.name}</p>
                          <p className="text-[10px] text-slate-400">{formatFileSize(fileObj.file.size)}</p>
                        </div>
                      </div>
                      <button
                        type="button"
            ...
            {uploadProgress > 0 && uploadProgress < 100 && (
              <div className="space-y-1">
                ...
                <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                  <div className="h-full bg-sky-600 transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                </div>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-100 rounded-2xl p-2 sm:p-6">
            ...
            <div className="flex items-center gap-2 sm:gap-4 my-2">
              ...
              <span className="text-[10px] sm:text-xs text-slate-400 font-medium">FDI</span>
              <div className="flex-1 h-0.5 bg-slate-300" />
            </div>
              ...
              <div className="space-y-1.5">
                {Object.values(
                  ...
                  <div
                    ...
                    className="flex items-center justify-between gap-2 bg-white rounded-lg shadow-sm border border-slate-100 px-2.5 py-1.5"
                  >
                    <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <p className="text-[10px] text-slate-400 flex items-center gap-1">
            <span className="text-slate-500">💡</span> <kbd className="px-1 py-0.5 bg-slate-100 border rounded text-[9px]">CTRL</kbd> + click per selezionare range
          </p>
          <div className="flex items-center gap-2">
            <Link
              to="/portal"
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 hover:text-slate-800 hover:bg-slate-100 transition-all"
            >
              <ArrowLeft size={12} />
              Annulla
            </Link>
            <button
              ...
              className="flex items-center gap-1 px-4 py-1.5 bg-sky-600 text-white rounded-lg text-xs font-semibold hover:bg-sky-700 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Check size={12} />
