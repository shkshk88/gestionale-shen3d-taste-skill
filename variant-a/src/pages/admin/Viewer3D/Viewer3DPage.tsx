...
export default function Viewer3DPage() {
  ...
  const getStatusBadge = (status: string) => {
    const map: Record<string, string> = {
      in_progress: 'bg-amber-50 text-amber-700 ring-1 ring-amber-500/20',
      received: 'bg-sky-50 text-sky-700 ring-1 ring-sky-500/20',
      qc: 'bg-violet-50 text-violet-700 ring-1 ring-violet-500/20',
      shipped: 'bg-teal-50 text-teal-700 ring-1 ring-teal-500/20',
    };
    const labelKey = `cases.statuses.${status}`;
    return (
      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${map[status] ?? 'bg-slate-100 text-slate-600 ring-1 ring-slate-400/20'}`}>
    ...
    <div className="space-y-6 animate-fade-in">
      {loading ? (
        ...
        <div className="h-[500px] flex flex-col items-center justify-center gap-4 text-slate-400">
          <div className="w-16 h-16 rounded-2xl bg-slate-50 flex items-center justify-center ring-1 ring-slate-100">
            <Box size={32} className="text-slate-300" />
          </div>
          <div className="text-center">
            <p className="text-sm font-medium text-slate-500">{t('viewer3d.noFilesAvailable', { defaultValue: 'Nessun caso con file 3D caricati' })}</p>
            <p className="text-xs text-slate-400 mt-1">{t('viewer3d.uploadToView', { defaultValue: 'Carica file STL o PLY su un caso per vederli qui' })}</p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
          ...
          <div className="lg:h-[calc(100vh-8rem)]">
            <div className="card-base p-4 flex flex-col lg:h-full">
              <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2 shrink-0 text-sm">
                <div className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center ring-1 ring-slate-200">
                    ...
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-2xl transition-all ${
                        active
                          ...
                          : 'bg-slate-50 hover:bg-slate-100 text-slate-700 ring-1 ring-slate-100'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`font-semibold text-sm truncate ${active ? 'text-white' : 'text-slate-800'}`} dir="auto">
                            {c.client?.studioName}
                          </p>
                          {c.patientName && (
                            <p className={`text-xs truncate ${active ? 'text-white/70' : 'text-slate-500'}`} dir="auto">
                              {c.patientName}
                            </p>
                          )}
                          {(firstType || teethList) && (
                            <p className={`text-[11px] truncate ${active ? 'text-white/50' : 'text-slate-400'}`}>
                ...
                <div className="p-4 border-b border-slate-100 flex items-center justify-between gap-3 shrink-0">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h2 className="font-bold text-slate-800 truncate text-sm" dir="auto">{selectedCase.client?.studioName}</h2>
                      {selectedCase.patientName && (
                        <p className="text-xs text-slate-500 truncate" dir="auto">{selectedCase.patientName}</p>
                      )}
                    </div>
                  </div>
                  <div className="shrink-0 flex items-center gap-2">
                    <div className="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center ring-1 ring-slate-100">
