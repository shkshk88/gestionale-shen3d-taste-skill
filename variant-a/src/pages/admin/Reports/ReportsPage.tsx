...
export default function ReportsPage() {
  ...
  const topClients: ClientRevenue[] = [
    { name: 'Clinica Dentale Rossi', revenue: 12450, cases: 45, color: 'bg-sky-500' },
    { name: 'Studio Dr. Verdi', revenue: 9820, cases: 32, color: 'bg-teal-500' },
    { name: 'Dental Care Center', revenue: 8340, cases: 28, color: 'bg-slate-700' },
    { name: 'Smile Center Ferrari', revenue: 6200, cases: 22, color: 'bg-slate-500' },
    { name: 'Studio Bianchi', revenue: 4890, cases: 18, color: 'bg-sky-600' },
  ];

  const totalClientRevenue = topClients.reduce((acc, c) => acc + c.revenue, 0);

  const materialUsage: MaterialUsage[] = [
    { id: 'zr', name: t('dental.materials.ZR'), count: 68, percentage: 35, color: 'bg-sky-500' },
    { id: 'emax', name: t('dental.materialsShort.EMAX'), count: 52, percentage: 27, color: 'bg-teal-500' },
    { id: 'mc', name: t('dental.materialsShort.METAL_CERAMIC'), count: 38, percentage: 20, color: 'bg-slate-700' },
    { id: 'pmma', name: t('dental.materials.PMMA'), count: 22, percentage: 11, color: 'bg-slate-500' },
    { id: 'comp', name: t('dental.materials.COMP'), count: 14, percentage: 7, color: 'bg-sky-600' },
  ];

  const casesByStatus = [
    { key: 'completed', status: t('reports.status_completed'), count: 124, color: 'bg-teal-500', percentage: 79 },
    { key: 'inProgress', status: t('reports.status_inProgress'), count: 22, color: 'bg-sky-500', percentage: 14 },
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">{t('reports.insightDesc')}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button className="px-4 py-2 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 transition-all flex items-center gap-2">
            <Download size={18} />
            {t('common.export')}
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="card-base p-5">
          <div className="flex items-start justify-between mb-3">
            <div className="w-11 h-11 rounded-xl bg-sky-500/20 flex items-center justify-center">
              <Euro size={22} className="text-sky-600" />
            </div>
            <span className={`flex items-center gap-1 text-sm font-medium ${
              stats.revenue.trend === 'up' ? 'text-teal-600' : 'text-red-600'
            }`}>
              {stats.revenue.trend === 'up' ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
              {stats.revenue.change}%
            </span>
          </div>
          <p className="text-sm text-slate-500 mb-1">{t('reports.revenue')}</p>
          <p className="text-2xl font-bold text-slate-800">₪{stats.revenue.value.toLocaleString('he-IL')}</p>
        </div>
        ...
        <div className="card-base p-5">
          <div className="flex items-start justify-between mb-3">
            ...
            <span className={`flex items-center gap-1 text-sm font-medium ${
              stats.cases.trend === 'up' ? 'text-teal-600' : 'text-red-600'
            }`}>
              {stats.cases.trend === 'up' ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
              {stats.cases.change}%
            </span>
          </div>
          <p className="text-sm text-slate-500 mb-1">{t('reports.completedCases')}</p>
          <p className="text-2xl font-bold text-slate-800">{stats.cases.value}</p>
        </div>

        {/* Average Value */}
        <div className="card-base p-5">
          <div className="flex items-start justify-between mb-3">
            <div className="w-11 h-11 rounded-xl bg-slate-800/20 flex items-center justify-center">
              <BarChart3 size={22} className="text-slate-800" />
            </div>
            <span className={`flex items-center gap-1 text-sm font-medium ${
              stats.avgValue.trend === 'up' ? 'text-teal-600' : 'text-red-600'
            }`}>
              {stats.avgValue.trend === 'up' ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
              {Math.abs(stats.avgValue.change)}%
            </span>
          </div>
          <p className="text-sm text-slate-500 mb-1">{t('reports.avgCaseValue')}</p>
          <p className="text-2xl font-bold text-slate-800">₪{stats.avgValue.value}</p>
        </div>

        {/* Active Clients */}
        <div className="card-base p-5">
          <div className="flex items-start justify-between mb-3">
            <div className="w-11 h-11 rounded-xl bg-slate-500/20 flex items-center justify-center">
              <Users size={22} className="text-slate-500" />
            </div>
            <span className="text-sm font-medium text-slate-400">—</span>
          </div>
          <p className="text-sm text-slate-500 mb-1">{t('clients.activeClients')}</p>
          <p className="text-2xl font-bold text-slate-800">{stats.clients.value}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 card-base p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-semibold text-slate-800">Andamento Fatturato</h3>
            ...
            <div className="flex items-center gap-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-600">Casi</span>
              </div>
            </div>
          </div>
            ...
            {monthlyData.map((data) => (
              <div key={data.monthKey} className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full flex flex-col items-center gap-1" style={{ height: '180px' }}>
                  {/* Revenue Bar */}
                  <div
                    className="w-full max-w-[40px] bg-sky-600 rounded-t-lg transition-all hover:opacity-80 relative group"
                ...
                <span className="text-sm text-slate-500">{t(`calendar.shortMonths.${data.monthKey}`)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card-base p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-semibold text-slate-800">Casi per Stato</h3>
          ...
          <div className="relative w-40 h-40 mx-auto mb-6">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-xs text-slate-500">{t('reports.total')}</p>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-base p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-semibold text-slate-800">Top Clienti per Fatturato</h3>
            <button className="text-sm text-sky-600 hover:underline">Vedi tutti</button>
          </div>

          <div className="space-y-4">
            {topClients.map((client, index) => (
              <div key={client.name} className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl ${rankColors[index] || 'bg-slate-400'} flex items-center justify-center text-white font-bold text-sm`}>
                  {index + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-slate-800 truncate">{client.name}</p>
                  <p className="text-sm text-slate-500">{t('reports.casesCount', { count: client.cases })}</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-slate-800">₪{client.revenue.toLocaleString('he-IL')}</p>
        ...
        <div className="card-base p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-semibold text-slate-800">Utilizzo Materiali</h3>
            <span className="text-sm text-slate-500">Questo mese</span>
          </div>
            ...
            {materialUsage.map((material) => (
              <div key={material.name}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-slate-500">{material.count} {t('reports.units')} ({material.percentage}%)</span>
                </div>
          ...
          <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
            <span className="text-sm font-medium text-slate-600">{t('reports.totalWork')}</span>
      ...
      {/* Quick Insights */}
      <div className="bg-gradient-to-r from-slate-800 to-slate-800/80 rounded-2xl p-6 text-white shadow-soft">
