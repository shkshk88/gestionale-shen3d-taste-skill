...
const typeConfig: Record<NotificationType, { icon: typeof Bell; color: string; bg: string; ring: string }> = {
  case:     { icon: Package,     color: 'text-teal-600', bg: 'bg-teal-50', ring: 'ring-teal-500/20' },
  message:  { icon: MessageSquare, color: 'text-sky-600', bg: 'bg-sky-50', ring: 'ring-sky-500/20' },
  payment:  { icon: Euro,        color: 'text-emerald-600', bg: 'bg-emerald-50', ring: 'ring-emerald-500/20' },
  alert:    { icon: AlertCircle, color: 'text-red-500', bg: 'bg-red-50', ring: 'ring-red-500/20' },
  system:   { icon: Bell,        color: 'text-slate-500', bg: 'bg-slate-100', ring: 'ring-slate-400/20' },
};
...
export default function NotificationsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* WIP banner */}
      <div className="rounded-2xl border border-amber-200 bg-amber-50/80 px-5 py-4 text-sm text-amber-900 shadow-soft">
        ...
        <div className="flex items-center gap-3">
          {unreadCount > 0 && (
            <button
              ...
              className="px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all flex items-center gap-2 shadow-soft text-sm font-medium"
            >
              <MailOpen size={16} />
              {t('notifications.markAllRead')}
            </button>
          )}
          <button className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all shadow-soft">
            <Settings size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="space-y-4">
          <div className="card-base p-4">
            <h3 className="font-semibold text-slate-800 mb-3 text-sm">{t('notifications.filterByType')}</h3>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all text-sm ${
                      active
                        ? 'bg-sky-50 text-sky-700 shadow-soft ring-1 ring-sky-500/20'
          ...
          {/* Quick Stats */}
          <div className="card-base p-4">
            <h3 className="font-semibold text-slate-800 mb-3 text-sm">{t('notifications.summary')}</h3>
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600">{t('notifications.unread')}</span>
                <span className="font-semibold text-sky-600">{unreadCount}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600">{t('notifications.today')}</span>
                <span className="font-semibold text-slate-800">
                  {notifications.filter(n => n.time.includes('min') || n.time.includes('h')).length}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600">{t('notifications.total')}</span>
        ...
        <div className="lg:col-span-3 space-y-3">
          {filteredNotifications.length > 0 ? (
              ...
              return (
                <div
                  ...
                  className={`card-base p-4 transition-all ${
                    !notification.read
                      ? 'ring-1 ring-sky-500/20 bg-sky-50/40'
                      : ''
                  }`}
                >
                  <div className="flex items-start gap-4">
                    <div className={`w-11 h-11 rounded-2xl ${config.bg} flex items-center justify-center flex-shrink-0 ring-1 ${config.ring}`}>
                      <Icon size={20} className={config.color} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <h4 className={`font-semibold text-sm ${!notification.read ? 'text-slate-800' : 'text-slate-600'}`}>
                            {notification.title}
                          </h4>
                          ...
                          <div className="flex items-center gap-4 mt-3">
                            ...
                            {notification.actionUrl && (
                              <button className="text-xs font-semibold text-sky-600 hover:text-sky-700 hover:underline transition-colors">
            ...
            <div className="card-base p-12 text-center">
              ...
              <p className="text-slate-500 font-medium">{t('notifications.noNotificationsInCategory')}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="card-base p-5 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-semibold text-slate-800">{t('notifications.notificationPreferences')}</h3>
            <p className="text-sm text-slate-500">{t('notifications.notificationPreferencesDesc')}</p>
          </div>
        </div>
        <button className="px-4 py-2.5 bg-slate-800 text-white rounded-xl text-sm font-semibold hover:bg-slate-700 transition-all shadow-soft">
