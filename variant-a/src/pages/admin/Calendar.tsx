    ...
    <div className="mt-1 flex items-center justify-center gap-1.5 flex-wrap">
      {shown.map((g, i) => (
        <div key={i} className="relative">
          ...
          />
          {g.count > 1 && (
            <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-[16px] px-0.5 rounded-full bg-sky-600 text-white text-[9px] font-bold flex items-center justify-center ring-2 ring-white">
              {g.count}
            </span>
          )}
        </div>
      ...
      {extra > 0 && (
        <span className={`text-[11px] font-bold ${dark ? 'text-white' : 'text-slate-600'}`}>+{extra}</span>
      )}
    </div>
  );
}
...
export default function CalendarPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-slate-500">{t('calendar.loadingDeliveries')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Calendar toolbar */}
      <div className="bg-white rounded-2xl border border-slate-100/80 shadow-soft p-5">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigateWeek('prev')}
              className="w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-slate-200 transition-all active:scale-[0.96]"
            >
              <ChevronLeft size={20} />
            </button>
            <h2 className="text-base sm:text-lg font-semibold text-slate-800 min-w-[170px] sm:min-w-[220px] text-center font-display">
              {getWeekRangeText()}
            </h2>
            <button
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              onClick={goToToday}
              className="px-4 py-2.5 bg-slate-50 rounded-2xl text-sm font-medium text-slate-600 hover:bg-slate-200 transition-colors active:scale-[0.98]"
            >
              {t('calendar.today')}
            </button>
            <div className="flex bg-slate-50 rounded-2xl p-1">
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-all ${
                  viewMode === 'week' ? 'bg-white shadow-soft text-slate-800' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                {t('calendar.week')}
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-all ${
                  viewMode === 'biweekly' ? 'bg-white shadow-soft text-slate-800' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                {t('calendar.twoWeeks')}
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-all ${
                  viewMode === 'month' ? 'bg-white shadow-soft text-slate-800' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                {t('calendar.month')}
              </button>
              ...
              {monthDays.map((date, i) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl md:rounded-2xl p-1.5 md:p-3 text-center transition-all min-h-[52px] md:min-h-[80px] ${
                      !isCurrentMonth
                  ...
                  >
                    <p className={`text-sm md:text-lg font-bold mb-1 ${
                      dayIsSelected ? 'text-white' : dayIsToday ? 'text-sky-600' : 'text-slate-800'
                    }`}>
                      {date.getDate()}
                    </p>
              ...
              {weekDays.slice(0, 7).map((date, i) => {
                ...
                return (
                  <button
                    key={i}
                    onClick={() => setSelectedDate(dateKey)}
                    className={`rounded-2xl md:rounded-2xl p-1.5 md:p-3 text-center transition-all ${
                      dayIsSelected
                        ? 'bg-sky-600 text-white shadow-card'
                        : dayIsToday
                        ? 'bg-sky-50 hover:bg-sky-100'
                  ...
                  >
                    <p className={`text-xs font-medium mb-1 ${
                      dayIsSelected ? 'text-white/80' : 'text-slate-400'
                    }`}>
            ...
            <div className="space-y-3">
              {selectedDeliveries.map((delivery) => (
                <Link
                  ...
                  className="bg-white rounded-2xl border border-slate-100/80 p-4 block hover:shadow-card-hover transition-all duration-200 group active:scale-[0.99]"
                >
                  <div className="flex items-center gap-3 mb-1.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-800 truncate">{delivery.client}</p>
                      <p className="text-xs text-slate-500 truncate">{delivery.patient}</p>
                    </div>
                    {getPriorityBadge(delivery.priority)}
                  </div>
                  <p className="text-xs text-slate-600 pl-12">
                    {delivery.type}
                    {delivery.material && delivery.material !== t('common.noData') && (
                      <span className="text-slate-400"> · {t(`dental.materials.${delivery.material}`, { defaultValue: delivery.material })}</span>
                    )}
                    {delivery.teeth && delivery.teeth !== t('common.noData') && (
                      <span className="text-slate-400"> · {delivery.teeth.split(', ').length} {t('dental.tooth', { count: delivery.teeth.split(', ').length })}</span>
                    )}
                  </p>
                  <p className="text-[10px] font-mono text-slate-400 pl-12 mt-1">#{delivery.caseNumber}</p>
                </Link>
              ))}
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-slate-100/80 p-8 text-center shadow-soft">
              <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center mx-auto mb-3">
                <Package size={22} className="text-slate-300" />
              </div>
              <p className="text-sm text-slate-400 font-medium">Nessuna consegna</p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400 bg-slate-100 px-2.5 py-1 rounded-full shrink-0">3gg</span>
            </div>
                ...
                if (upcomingDeliveries.length === 0) {
                  return (
                    <div className="bg-white rounded-2xl border border-slate-100/80 p-8 text-center shadow-soft">
                      ...
                      <p className="text-sm text-slate-400 font-medium">Nessun lavoro in uscita</p>
                    </div>
                  );
                }
                    ...
                    >
                      <div className="flex items-center gap-3 mb-1.5">
                        ...
                        <span className={`text-[10px] font-semibold px-2 py-1 rounded-lg shrink-0 ${
                          daysDiff === 0 ? 'bg-red-50 text-red-700' : daysDiff === 1 ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-600'
                        }`}>{daysText}</span>
                      </div>
                      <p className="text-xs text-slate-600 pl-12">
                        {delivery.type}
                        ...
                        {delivery.teeth && delivery.teeth !== t('common.noData') && (
                          <span className="text-slate-400"> · {delivery.teeth.split(', ').length}d</span>
                        )}
                      </p>
                      <p className="text-[10px] font-mono text-slate-400 pl-12 mt-1">#{delivery.caseNumber}</p>
                    </Link>
                  ...
                  >
                    <div className="flex items-center gap-3 mb-1.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 truncate">{c.patient}</p>
                      </div>
                      <span className={`text-[10px] font-semibold px-2 py-1 rounded-lg shrink-0 ${
                        c.daysSince >= 7 ? 'bg-red-50 text-red-700' : c.daysSince >= 3 ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-600'
                      }`}>
                    ...
                    <p className="text-xs text-slate-600 pl-12">
                      ...
                      {c.material && (
                        <span className="text-slate-400"> · {t(`dental.materials.${c.material}`, { defaultValue: c.material })}</span>
                      )}
                      {c.teethCount > 0 && (
                        <span className="text-slate-400"> · {c.teethCount}d</span>
                      )}
                    </p>
                    <p className="text-[10px] font-mono text-slate-400 pl-12 mt-1">#{c.caseNumber}</p>
                  </Link>
                ));
              })()}
            </div>
