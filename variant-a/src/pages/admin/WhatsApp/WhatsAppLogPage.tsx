...
export default function WhatsAppLogPage() {
  ...
  const statusBadge = (s: string, shadow: boolean) => {
    ...
    const colors: Record<string, string> = {
      queued: 'bg-slate-100 text-slate-700 ring-1 ring-slate-400/20',
      sent: 'bg-sky-50 text-sky-700 ring-1 ring-sky-500/20',
      delivered: 'bg-teal-50 text-teal-700 ring-1 ring-teal-500/20',
      read: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-500/20',
      failed: 'bg-red-50 text-red-700 ring-1 ring-red-500/20',
    };
    return (
      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${colors[s] || 'bg-slate-100 text-slate-700 ring-1 ring-slate-400/20'}`}>
        {s}
      </span>
    );
  };
    ...
    <div className="space-y-6 animate-fade-in pb-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-2xl bg-slate-800 flex items-center justify-center shadow-soft">
            <MessageCircle size={20} className="text-white" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-slate-800">WhatsApp Agent</h1>
        ...
        <button
          onClick={loadAll}
          disabled={loading}
          className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-white border border-slate-200 text-sm text-slate-700 hover:bg-slate-50 hover:border-slate-300 shadow-soft transition-all"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card-base p-5">
          <p className="text-xs text-slate-500 mb-2 font-medium uppercase tracking-wide">Modalità invio</p>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {status?.autoSendEnabled ? (
                ...
                <>
                  <EyeOff size={18} className="text-sky-600" />
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-semibold transition shadow-soft ${
                status?.autoSendEnabled
                  ...
                  : 'bg-teal-50 text-teal-700 hover:bg-teal-100 ring-1 ring-teal-500/20'
              }`}
            >
        ...
        <div className="card-base p-5">
          <p className="text-xs text-slate-500 mb-2 font-medium uppercase tracking-wide">Meta API</p>
          <div className="flex items-center gap-2">
        ...
        <div className="card-base p-5">
          <p className="text-xs text-slate-500 mb-2 font-medium uppercase tracking-wide">Template attivi</p>
          <div className="flex items-center gap-2">
      ...
      <div className="card-base p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          {templates.length > 4 && (
            <button
              ...
              className="text-xs text-sky-600 font-semibold hover:text-sky-700 transition-colors flex items-center gap-1"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {visibleTemplates.map((tpl) => (
            <div
              key={tpl.id}
              className="border border-slate-100 rounded-2xl p-4 bg-slate-50/50 hover:bg-white hover:shadow-soft transition-all"
            >
              <div className="flex items-center justify-between mb-2">
                <code className="text-xs font-mono font-semibold text-slate-800 bg-slate-100 px-2 py-1 rounded-lg">
                  {tpl.name}
                </code>
                <span
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {tpl.variables.map((v, i) => (
                  <span
                    key={i}
                    className="text-[10px] px-2 py-0.5 bg-sky-50 text-sky-700 rounded-lg font-mono ring-1 ring-sky-500/20"
                  >
                    {`{{${i + 1}}}`}={v}
                  </span>
                ))}
      ...
      <div className="card-base p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={loadAll}
              className="px-3 py-1.5 text-xs rounded-xl bg-sky-600 text-white font-semibold hover:bg-sky-700 transition-colors shadow-soft"
            >
              Applica
            </button>
          </div>
        ...
        {messages.length === 0 ? (
          <div className="text-center py-10 text-slate-400 text-sm">
            <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center mx-auto mb-3 ring-1 ring-slate-100">
              ...
              <tbody className="divide-y divide-slate-50">
                {messages.map((m) => (
                  ...
                  >
                      ...
                      <span
                        className={`px-2 py-1 rounded-lg text-[10px] font-semibold ${
                          m.direction === 'outbound'
                            ? 'bg-sky-50 text-sky-700 ring-1 ring-sky-500/20'
                            : 'bg-teal-50 text-teal-700 ring-1 ring-teal-500/20'
                        }`}
                      >
                    ...
                    <td className="py-3 px-3">
                      <code className="text-[11px] text-slate-700 bg-slate-100 px-1.5 py-0.5 rounded">
                        {m.templateName || '—'}
                      </code>
                    </td>
