...
export default function ClientList() {
  ...
  const getPriorityBadgeClass = (priority: ClientPriority) => {
    switch (priority) {
      ...
      case 'normal': return 'bg-sky-50 text-sky-700 border border-sky-200';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-end gap-2">
        <Link to="/admin/settings" className="inline-flex items-center gap-1 rounded-full bg-slate-800 px-3 py-1.5 text-xs font-medium text-white shadow-sm transition hover:bg-slate-700">
          <Settings size={14} />
          {t('settings.title')}
        </Link>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="relative flex-1 max-w-md w-full">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-slate-200 bg-white py-2.5 pl-11 pr-4 text-sm text-slate-800 shadow-sm outline-none transition focus:border-sky-600 focus:ring-2 focus:ring-sky-600/20"
          />
        </div>
        {/* View mode toggle */}
        <div className="inline-flex items-center rounded-2xl bg-slate-100 p-0.5 shadow-sm">
          <button
        ...
        <Link to="/admin/clients/new" className="inline-flex items-center gap-2 rounded-2xl bg-sky-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-sky-700 whitespace-nowrap">
          <Plus size={18} />
          {t('clients.newClient')}
        </Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-2xl bg-white p-4 shadow-sm border border-slate-100">
          <p className="text-xs text-slate-500 mb-1">{t('clients.totalClients')}</p>
          <p className="text-xl font-bold text-slate-800">{totalStats.clients}</p>
        </div>
        <div className="rounded-2xl bg-white p-4 shadow-sm border border-slate-100">
          <p className="text-xs text-slate-500 mb-1">{t('clients.totalCases')}</p>
          <p className="text-xl font-bold text-slate-800">{totalStats.totalCases}</p>
        </div>
        <div className="rounded-2xl bg-white p-4 shadow-sm border border-slate-100">
          <p className="text-xs text-slate-500 mb-1">{t('clients.active')}</p>
          <p className="text-xl font-bold text-teal-500">{totalStats.activeCases}</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredClients.map((client) => (
            <div
              ...
              className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100 hover:shadow-md transition-all group cursor-pointer relative"
            >
              {/* Header */}
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-sm text-slate-800 truncate" title={client.studioName}>{client.studioName}</h3>
                  <p className="text-xs text-slate-500 truncate">{client.contactPerson}</p>
                </div>
                <ChevronRight size={16} className="text-slate-300 group-hover:text-sky-600 transition-colors shrink-0" />
              </div>
              ...
              <div className="flex items-center gap-2 mb-4">
                <div data-dropdown onClick={(e) => e.stopPropagation()}>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="w-32 rounded-2xl shadow-md border border-slate-100">
                      {(['active', 'inactive', 'blocked'] as ClientStatus[]).map((status) => (
                        ...
                        >
                          ...
                          {client.status === status && <span className="ml-auto text-sky-600">✓</span>}
                        </DropdownMenuItem>
                      ))}
                    </DropdownMenuContent>
                  </DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="w-32 rounded-2xl shadow-md border border-slate-100">
                      {(['high', 'normal', 'low'] as ClientPriority[]).map((priority) => (
                        ...
                        >
                          ...
                          {client.priority === priority && <span className="ml-auto text-sky-600">✓</span>}
                        </DropdownMenuItem>
                      ))}
                    </DropdownMenuContent>
                  </DropdownMenu>
              ...
              <div className="flex items-center gap-3 text-xs text-slate-500 mb-4">
                <span className="flex items-center gap-1 truncate">
                  <Phone size={12} className="shrink-0 text-sky-600" />
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs font-medium text-slate-600">{client.totalCases}</span>
                  </div>
                  {client.activeCases > 0 && (
                    <div className="flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-teal-500"></span>
                      <span className="text-xs font-medium text-teal-500">{client.activeCases}</span>
                    </div>
                  )}
                </div>
                <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                  <div data-dropdown onClick={(e) => e.stopPropagation()}>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="w-7 h-7 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 hover:text-sky-600 transition-colors">
                          <MoreHorizontal size={14} />
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="rounded-2xl shadow-md border border-slate-100">
              ...
              <tbody>
                {filteredClients.map((client) => (
                  <tr
                    key={client.id}
                    onClick={(e) => handleCardClick(client.id, e)}
                    className="border-t border-slate-100 hover:bg-slate-50/50 cursor-pointer transition group"
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <p className="font-medium text-slate-800 truncate">{client.studioName}</p>
                          <p className="text-[11px] text-slate-500 truncate md:hidden">{client.contactPerson}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3 hidden lg:table-cell text-slate-600 max-w-[200px] truncate">
                      <span className="inline-flex items-center gap-1.5 text-xs">
                        {client.email ? (
                          <>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div data-dropdown onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button className="w-7 h-7 rounded-xl bg-slate-50 hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-sky-600 transition opacity-0 group-hover:opacity-100">
                              <MoreHorizontal size={14} />
                            </button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end" className="rounded-2xl shadow-md border border-slate-100">
      ...
      {!loading && filteredClients.length === 0 && (
        <div className="rounded-2xl bg-white py-12 text-center shadow-sm border border-slate-100">
