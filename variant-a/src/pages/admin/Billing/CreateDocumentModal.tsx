      ...
      >
        ...
        <div className="flex items-center gap-3 p-4 border-b border-slate-100 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-slate-400 uppercase tracking-wide">{t('billing.create_modal.title')}</p>
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-4">
          ...
          <div>
            ...
            <div className="rounded-xl border border-slate-100 overflow-hidden divide-y divide-neutral-100 max-h-44 overflow-y-auto">
              {cases.map((c) => (
                <label
                  key={c.id}
                ...
                >
                  ...
                  />
                  <span className="font-mono text-xs text-sky-600">{c.caseNumber}</span>
                  {c.patientName && (
                    <span className="text-xs text-slate-500 truncate flex-1">— {c.patientName}</span>
                  )}
                  <span className="text-xs font-semibold text-slate-700 shrink-0 ml-auto">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="space-y-2">
              ...
              <div>
                <label className="text-xs font-medium text-slate-600 mb-1 block">IVA %</label>
                <input
                  type="number"
                  value={taxRate}
                  min={0}
        ...
        {/* Footer */}
        <div className="border-t border-slate-100 p-4 bg-slate-50/60 shrink-0">
          ...
          <div className="flex gap-2">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-100"
            >
              {t('common.cancel')}
            </button>
            <button
              ...
              className="flex-[2] py-2.5 rounded-xl bg-sky-600 text-white text-sm font-semibold flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
            >
